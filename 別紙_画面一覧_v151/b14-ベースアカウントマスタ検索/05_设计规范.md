# b14-ベースアカウントマスタ検索 - 设计规范

## 1. 功能概述

### 1.1 文档信息

| 项目 | 内容 |
|------|------|
| **工作表名称** | b14-ベースアカウントマスタ検索 |
| **界面编号** | b14 |
| **对应Base界面** | b11-輸送パートナーマスタ一覧 |
| **文档版本** | v1.0 |
| **最后更新** | 2026-02-XX|
| **文档状态** | ✅ 已发布 |

### 1.2 功能描述

b14-ベースアカウントマスタ検索是ベース（总部）侧的基础账户搜索页面。管理者可以通过此页面：

- 根据多条件搜索基础账户（ユーザーID、メール、ユーザー名、所属、権限）
- 从AzureAD同步用户信息
- 查看搜索结果列表
- 查看账户详细信息（跳转到b15）
- 新規追加基础账户

### 1.3 技术架构

本文档基于 **前后端分离架构**，详细技术规范请参考：

👉 **[\_公共技术架构规范.md](../_公共技术架构规范.md)**

| 层级 | 技术 |
|------|------|
| 前端 | React 18.x + TypeScript + Ant Design |
| 后端 | Spring Boot 3.x + Java 17 |
| 数据库 | PostgreSQL 15.x |

### 1.4 核心功能

| 功能 | 描述 | 优先级 |
|------|------|--------|
| 多条件搜索 | 支持5个字段的组合搜索 | P0 |
| AzureAD連携 | 从AzureAD获取用户信息 | P0 |
| 検索結果表示 | 表格展示搜索结果 | P0 |
| 詳細表示 | 跳转到账户详情页面 | P0 |
| 新規追加 | 新增基础账户 | P0 |
| 並び替え | 各列排序支持 | P0 |
| ページング | 分页展示结果 | P0 |

### 1.5 适用范围

- 前端开发人员（搜索页面组件实现）
- 后端开发人员（账户API开发）
- 测试工程师（功能测试）

---

## 2. 页面元素

### 2.1 元素清单

| 序号 | 元素名称 | 元素类型 | 功能说明 | 交互触发 |
|------|----------|----------|----------|----------|
| ⑴ | 検索条件エリア | Form | 検索条件入力エリア | フォーム入力 |
| ⑵ | ユーザーID入力 | Input | AzureADユーザーID | 文字入力 |
| ⑶ | メールアドレス入力 | Input | AzureADメールアドレス | 文字入力 |
| ⑷ | ユーザー名入力 | Input | AzureADユーザー名 | 文字入力 |
| ⑸ | 所属入力 | Input | 所属ベース名 | 文字入力 |
| ⑹ | 権限選択 | Radio | 権限区分選択 | ラジオ選択 |
| ⑺ | 検索ボタン | Primary Button | 検索実行 | クリック |
| ⑻ | リセットボタン | Default Button | 条件リセット | クリック |
| ⑼ | 検索結果エリア | Card | 検索結果表示エリア | 表示 |
| ⑽ | ＋新規追加 | Button | 新規アカウント追加 | クリック |
| ⑾ | 結果テーブル | Table | アカウント一覧 | 行クリック/ソート |
| ⑿ | ユーザーID列 | Link Column | ユーザーID | クリック→詳細 |
| ⒀ | ユーザー名列 | Column | AzureAD用户名 | 表示/ソート |
| ⒁ | 所属列 | Column | 所属ベース名 | 表示/ソート |
| ⒂ | 権限列 | Tag Column | 権限区分 | Tag表示 |
| ⒃ | アカウント状態列 | Tag Column | 有効/無効 | Tag表示 |
| ⒄ | メールアドレス列 | Column | メールドレス | 表示/ソート |
| ⒅ | ページネーション | Pagination | ページング操作 | クリック |
| ⒆ | 合計表示 | Statistic | 検索結果合計 | 表示 |

### 2.2 検索条件詳細説明

| フィールド | 要素タイプ | 制約 | データ形式 | デフォルト |
|-----------|-----------|------|-----------|----------|
| ユーザーID | Input | 半角英数50文字 | 部分一致 | 空 |
| メールアドレス | Input | email形式200文字 | 部分一致 | 空 |
| ユーザー名 | Input | 文字列200文字 | 部分一致 | 空 |
| 所属 | Input | 文字列200文字 | 部分一致 | 空 |
| 権限 | Radio | 列挙値 | 完全一致 | 空 |

### 2.3 検索結果テーブル詳細説明

| カラム名 | 要素タイプ | 書式 | ソート | 説明 |
|----------|-----------|------|--------|------|
| ユーザーID | Link | 文字列 | ○ | 詳細ページへ |
| ユーザー名 | Text | 文字列 | ○ | AzureAD用户名 |
| 所属 | Text | 文字列 | ○ | 所属ベース名 |
| 権限 | Tag | 色付き | ○ | 権限区分 |
| アカウント状態 | Tag | 色付き | ○ | 状態表示 |
| メールアドレス | Text | email形式 | ○ | メールドレス |

### 2.4 権限値詳細

| 権限値 | 表示名 | タグ色 | 説明 |
|--------|--------|--------|------|
| 管理者 | 管理者 | 赤色 | システム管理者 |
| 役割者 | 役割者 | 青色 | 一般役割者 |
| 配車担当者 | 配車担当者 | 緑色 | 配車業務担当者 |
| 計上担当者 | 計上担当者 | 橙色 | 計上業務担当者 |

### 2.5 ステータス値詳細

| ステータス値 | 表示名 | タグ色 | 説明 |
|--------------|--------|--------|------|
| 有効 | アカウント有効 | 緑色 | 账户正常使用中 |
| 無効 | アカウント無効 | 赤色 | 账户已禁用 |

---

## 3. 交互流程

### 3.1 ページロードフロー

| ステップ | アクション | システム応答 |
|----------|-----------|--------------|
| 1 | ユーザーがb14ページへ遷移 | ページ初期化 |
| 2 | 空の検索条件設定 | すべて空欄 |
| 3 | デフォルト検索実行 | 全件検索 |
| 4 | 検索API自動実行 | GET /api/v1/base-accounts/search |
| 5 | APIレスポンス受信 | テーブルデータ設定 |
| 6 | テーブル描画 | ページネーション設定 |
| 7 | ページ表示完了 | ユーザー操作待機 |

### 3.2 検索操作フロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | 検索条件入力/選択 | リアルタイムバリデーション |
| 2 | 検索ボタンクリック | ローディング表示開始 |
| 3 | 検索パラメータ構築 | 各フィールド値取得 |
| 4 | APIリクエスト送信 | GET /api/v1/base-accounts/search |
| 5 | レスポンス受信 | テーブルデータ更新 |
| 6 | ページネーション更新 | 合計件数表示更新 |
| 7 | 検索完了 | ローディング表示終了 |

### 3.3 リセット操作フロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | リセットボタンクリック | 確認ダイアログ表示 |
| 2 | 確認 | 全検索条件クリア |
| 3 | 検索API再実行 | page=1で全件検索 |
| 4 | テーブル更新 | 初期状態表示 |

### 3.4 詳細表示フロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | ユーザーIDリンククリック | b15页面へ遷移 |
| 2 | URLパラメータ設定 | /b15/{userId} |
| 3 | 詳細ページロード | アカウント情報表示 |

### 3.5 新規追加フロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | ＋新規追加ボタンクリック | AzureAD検索モーダル表示 |
| 2 | AzureADユーザー検索 | AzureAD API実行 |
| 3 | ユーザー選択 | 情報取得・表示 |
| 4 | 権限・所属設定 | 各項目入力 |
| 5 | 保存ボタンクリック | 新規登録API実行 |

### 3.6 ページングフロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | ページ番号クリック | ページ番号取得 |
| 2 | 検索API再実行 | pageパラメータ追加 |
| 3 | テーブル更新 | 該当ページデータ表示 |
| 4 | スクロール位置調整 | テーブル上部へスクロール |

### 3.7 並び替えフロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | 列ヘッダークリック | ソート順序切替 |
| 2 | ソートパラメータ追加 | sort, sortOrder |
| 3 | 検索API再実行 | 並び替え適用 |
| 4 | テーブル更新 | 並び替え後データ表示 |

---

## 4. 界面规范

### 4.1 ページレイアウト

| 属性 | PC端値 | SP端値 | 説明 |
|------|--------|--------|------|
| **ページタイプ** | PCのみ | - | PC专用页面 |
| **レイアウト方式** | 上→下配置 | 垂直配置 | 検索→テーブル→ページング |
| **検索エリア** | 水平配置 | 垂直配置 | PC:5列, SP:縦1列 |

### 4.2 検索エリア仕様

| 属性 | 値 | 説明 |
|------|----|------|
| **ラベル幅** | 100px | 固定幅 |
| **入力欄幅** | 200px | PC端固定幅 |
| **ラジオ間隔** | 16px | オプション間余白 |
| **ボタン間隔** | 8px | ボタン間余白 |
| **フォーム余白** | 24px | 検索エリア下部余白 |

### 4.3 テーブル仕様

| 属性 | 値 | 説明 |
|------|----|------|
| **行高** | 48px | 固定行高 |
| **フォントサイズ** | 14px | 標準テキスト |
| **縞模様** | 有り | 偶数行背景#fafafa |
| **ホバー効果** | 有り | 行ホバー背景#f5f5f5 |
| **ボーダー** | 1px #e8e8e8 | セル境界線 |
| **ヘッダー固定** | 有り | スクロール時ヘッダー固定 |

### 4.4 ステータスタグ仕様

| ステータス | 背景色 | 文字色 | 説明 |
|-----------|--------|--------|------|
| 有効 | #52c41a | #ffffff | 绿色标签 |
| 無効 | #f5222d | #ffffff | 红色标签 |
| 管理者 | #f5222d | #ffffff | 赤色标签 |
| 役割者 | #1890ff | #ffffff | 蓝色标签 |
| 配車担当者 | #52c41a | #ffffff | 绿色标签 |
| 計上担当者 | #fa8c16 | #ffffff | 橙色标签 |

### 4.5 ページネーション仕様

| 属性 | 値 | 説明 |
|------|----|------|
| **ページサイズ** | 10/20/50/100 | 選択可能 |
| **表示位置** | 右下 | テーブル下中央 |
| **総件数表示** | 有り | "1-20 / 100件" 形式 |

---

## 5. API接口设计

### 5.1 アカウント検索API

```typescript
// GET /api/v1/base-accounts/search
interface SearchBaseAccountsRequest {
  userId?: string;
  email?: string;
  userName?: string;
  affiliation?: string;
  permission?: PermissionType;
  page?: number;
  pageSize?: number;
  sort?: string;
  sortOrder?: 'asc' | 'desc';
}

interface SearchBaseAccountsResponse {
  success: boolean;
  data: {
    accounts: BaseAccountInfo[];
    pagination: {
      currentPage: number;
      pageSize: number;
      totalItems: number;
      totalPages: number;
    };
    summary: {
      totalCount: number;
      adminCount: number;
      operatorCount: number;
    };
  };
  error?: {
    code: string;
    message: string;
    details?: Record<string, unknown>;
  };
}

type PermissionType = '管理者' | '役割者' | '配車担当者' | '計上担当者';

interface BaseAccountInfo {
  userId: string;
  userName: string;
  affiliation: string;
  permission: PermissionType;
  accountStatus: '有効' | '無効';
  email: string;
  createdAt: string;
  lastLoginAt?: string;
}
```

### 5.2 アカウント詳細取得API

```typescript
// GET /api/v1/base-accounts/{userId}
interface GetBaseAccountDetailResponse {
  success: boolean;
  data: {
    userId: string;
    userName: string;
    email: string;
    affiliation: string;
    permission: PermissionType;
    accountStatus: '有効' | '無効';
    azureAdId: string;
    roles: string[];
    permissions: string[];
    createdAt: string;
    updatedAt: string;
    lastLoginAt?: string;
  };
}
```

### 5.3 AzureADユーザー検索API

```typescript
// GET /api/v1/external/azure-ad/users/search
interface SearchAzureADUsersRequest {
  query?: string;
  department?: string;
  page?: number;
  pageSize?: number;
}

interface SearchAzureADUsersResponse {
  success: boolean;
  data: {
    users: AzureADUserInfo[];
    pagination: {
      currentPage: number;
      pageSize: number;
      totalItems: number;
    };
  };
}

interface AzureADUserInfo {
  userId: string;
  userName: string;
  email: string;
  department?: string;
  jobTitle?: string;
}
```

### 5.4 アカウント作成API

```typescript
// POST /api/v1/base-accounts
interface CreateBaseAccountRequest {
  userId: string;
  userName: string;
  email: string;
  affiliation: string;
  permission: PermissionType;
  accountStatus: '有効' | '無効';
  roles?: string[];
}

interface CreateBaseAccountResponse {
  success: boolean;
  data: {
    userId: string;
    createdAt: string;
  };
  message: string;
}
```

### 5.5 APIエラーレスポンス

```typescript
interface ApiErrorResponse {
  success: false;
  error: {
    code: ErrorCode;
    message: string;
    details?: Record<string, unknown>;
  };
}

type ErrorCode =
  | 'INVALID_PARAMETER'
  | 'ACCOUNT_NOT_FOUND'
  | 'DUPLICATE_ACCOUNT'
  | 'PERMISSION_DENIED'
  | 'EXTERNAL_SYSTEM_ERROR'
  | 'INVALID_SEARCH_CONDITION'
  | 'RATE_LIMIT_EXCEEDED'
  | 'INTERNAL_ERROR';
```

---

## 6. 前端实现

### 6.1 メインコンポーネント

```typescript
// BaseAccountSearchPage.tsx
import React, { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import {
  Form,
  Input,
  Radio,
  Button,
  Table,
  Tag,
  Pagination,
  Space,
  Card,
  Statistic,
  Row,
  Col,
  message,
  Tooltip,
} from 'antd';
import {
  SearchOutlined,
  PlusOutlined,
  ReloadOutlined,
  FilterOutlined,
} from '@ant-design/icons';
import { BaseAccountInfo, SearchBaseAccountsRequest, PermissionType } from '../../types/baseAccount';
import { searchBaseAccounts, getAzureADUsers } from '../../api/baseAccount';
import { PermissionTag } from '../../components/PermissionTag';
import { StatusTag } from '../../components/StatusTag';
import styles from './BaseAccountSearchPage.module.css';

const { RangePicker } = DatePicker;

type PermissionOption = '管理者' | '役割者' | '配車担当者' | '計上担当者';

export const BaseAccountSearchPage: React.FC = () => {
  const [form] = Form.useForm();
  const navigate = useNavigate();
  const [data, setData] = useState<BaseAccountInfo[]>([]);
  const [loading, setLoading] = useState(false);
  const [pagination, setPagination] = useState({
    current: 1,
    pageSize: 20,
    total: 0,
  });
  const [summary, setSummary] = useState({
    totalCount: 0,
    adminCount: 0,
    operatorCount: 0,
  });
  const [filters, setFilters] = useState<SearchBaseAccountsRequest>({});

  const fetchData = useCallback(async (params: SearchBaseAccountsRequest) => {
    setLoading(true);
    try {
      const response = await searchBaseAccounts({
        ...params,
        page: params.page || 1,
        pageSize: params.pageSize || 20,
      });

      if (response.success) {
        setData(response.data.accounts);
        setPagination(prev => ({
          ...prev,
          total: response.data.pagination.totalItems,
        }));
        setSummary({
          totalCount: response.data.summary.totalCount,
          adminCount: response.data.summary.adminCount,
          operatorCount: response.data.summary.operatorCount,
        });
      } else {
        message.error(response.error?.message || '検索に失敗しました');
      }
    } catch (error) {
      message.error('ネットワークエラーが発生しました');
      console.error('Search error:', error);
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => {
    fetchData({ page: 1 });
  }, [fetchData]);

  const handleSearch = (values: any) => {
    const searchParams: SearchBaseAccountsRequest = {
      ...values,
      page: 1,
      pageSize: pagination.pageSize,
    };
    setFilters(searchParams);
    fetchData(searchParams);
  };

  const handleReset = () => {
    form.resetFields();
    setFilters({});
    fetchData({ page: 1, pageSize: pagination.pageSize });
  };

  const handlePageChange = (page: number, pageSize?: number) => {
    setPagination(prev => ({
      ...prev,
      current: page,
      pageSize: pageSize || prev.pageSize,
    }));
    fetchData({
      ...filters,
      page,
      pageSize: pageSize || pagination.pageSize,
    });
  };

  const handleViewDetail = (userId: string) => {
    navigate(`/b15/${userId}`);
  };

  const handleAddAccount = () => {
    navigate('/b14/new');
  };

  const columns = [
    {
      title: 'ユーザーID',
      dataIndex: 'userId',
      key: 'userId',
      width: 150,
      sorter: true,
      render: (userId: string) => (
        <Button
          type="link"
          onClick={() => handleViewDetail(userId)}
          className={styles.userIdLink}
        >
          {userId}
        </Button>
      ),
    },
    {
      title: 'ユーザー名',
      dataIndex: 'userName',
      key: 'userName',
      width: 180,
      sorter: true,
      render: (text: string) => (
        <Tooltip title={text}>
          <span className={styles.truncateText}>{text}</span>
        </Tooltip>
      ),
    },
    {
      title: '所属',
      dataIndex: 'affiliation',
      key: 'affiliation',
      width: 150,
      sorter: true,
      render: (text: string) => (
        <Tooltip title={text}>
          <span className={styles.truncateText}>{text}</span>
        </Tooltip>
      ),
    },
    {
      title: '権限',
      dataIndex: 'permission',
      key: 'permission',
      width: 120,
      sorter: true,
      render: (permission: PermissionType) => (
        <PermissionTag permission={permission} />
      ),
    },
    {
      title: 'アカウント状態',
      dataIndex: 'accountStatus',
      key: 'accountStatus',
      width: 120,
      sorter: true,
      render: (status: string) => <StatusTag status={status} />,
    },
    {
      title: 'メールアドレス',
      dataIndex: 'email',
      key: 'email',
      width: 250,
      sorter: true,
      render: (email: string) => (
        <Tooltip title={email}>
          <span className={styles.truncateText}>{email}</span>
        </Tooltip>
      ),
    },
  ];

  return (
    <div className={styles.container}>
      <Card title="ベースアカウントマスタ検索" className={styles.searchCard}>
        <Form
          form={form}
          layout="horizontal"
          onFinish={handleSearch}
          initialValues={{
            permission: undefined,
          }}
        >
          <Row gutter={16}>
            <Col xs={24} sm={12} md={8} lg={6}>
              <Form.Item
                name="userId"
                label="ユーザーID"
                className={styles.formItem}
              >
                <Input
                  placeholder="AzureADのユーザーIDを入力"
                  allowClear
                  maxLength={50}
                />
              </Form.Item>
            </Col>

            <Col xs={24} sm={12} md={8} lg={6}>
              <Form.Item
                name="email"
                label="メールアドレス"
                className={styles.formItem}
              >
                <Input
                  placeholder="AzureADのメールアドレスを入力"
                  allowClear
                  maxLength={200}
                />
              </Form.Item>
            </Col>

            <Col xs={24} sm={12} md={8} lg={6}>
              <Form.Item
                name="userName"
                label="ユーザー名"
                className={styles.formItem}
              >
                <Input
                  placeholder="AzureADのユーザー名を入力"
                  allowClear
                  maxLength={200}
                />
              </Form.Item>
            </Col>

            <Col xs={24} sm={12} md={8} lg={6}>
              <Form.Item
                name="affiliation"
                label="所属"
                className={styles.formItem}
              >
                <Input
                  placeholder="所属ベース名を入力"
                  allowClear
                  maxLength={200}
                />
              </Form.Item>
            </Col>

            <Col xs={24} sm={12} md={8} lg={6}>
              <Form.Item
                name="permission"
                label="権限"
                className={styles.formItem}
              >
                <Radio.Group>
                  <Radio value="管理者">管理者</Radio>
                  <Radio value="役割者">役割者</Radio>
                  <Radio value="配車担当者">配車担当者</Radio>
                  <Radio value="計上担当者">計上担当者</Radio>
                </Radio.Group>
              </Form.Item>
            </Col>
          </Row>

          <div className={styles.buttonGroup}>
            <Button
              type="primary"
              htmlType="submit"
              icon={<SearchOutlined />}
              loading={loading}
            >
              検索
            </Button>
            <Button onClick={handleReset}>
              リセット
            </Button>
            <Button
              icon={<ReloadOutlined />}
              onClick={() => fetchData(filters)}
              loading={loading}
            >
              再読込
            </Button>
          </div>
        </Form>
      </Card>

      {summary.totalCount > 0 && (
        <div className={styles.summary}>
          <Row gutter={16}>
            <Col span={6}>
              <Statistic
                title="合計件数"
                value={summary.totalCount}
                suffix="件"
              />
            </Col>
            <Col span={6}>
              <Statistic
                title="管理者"
                value={summary.adminCount}
                valueStyle={{ color: '#f5222d' }}
              />
            </Col>
            <Col span={6}>
              <Statistic
                title="一般ユーザー"
                value={summary.operatorCount}
                valueStyle={{ color: '#1890ff' }}
              />
            </Col>
          </Row>
        </div>
      )}

      <Card
        className={styles.resultCard}
        extra={
          <Button
            type="primary"
            icon={<PlusOutlined />}
            onClick={handleAddAccount}
          >
            ＋新規追加
          </Button>
        }
      >
        <Table
          columns={columns}
          dataSource={data}
          rowKey="userId"
          loading={loading}
          pagination={false}
          scroll={{ x: 1000 }}
          locale={{
            emptyText: '検索条件に一致するアカウントがありません',
          }}
          onChange={(paginationConfig, filters, sorter) => {
            const sortField = Array.isArray(sorter) ? sorter[0]?.field : sorter.field;
            const sortOrder = Array.isArray(sorter) ? sorter[0]?.order : sorter.order;
            
            if (sortField && sortOrder) {
              fetchData({
                ...filters,
                sort: sortField as string,
                sortOrder: sortOrder === 'ascend' ? 'asc' : 'desc',
              });
            }
          }}
        />

        {pagination.total > 0 && (
          <div className={styles.pagination}>
            <Pagination
              current={pagination.current}
              pageSize={pagination.pageSize}
              total={pagination.total}
              onChange={handlePageChange}
              onShowSizeChange={handlePageChange}
              showSizeChanger
              showQuickJumper
              showTotal={(total, range) => (
                `${range[0]}-${range[1]} / ${total}件`
              )}
              pageSizeOptions={['10', '20', '50', '100']}
            />
          </div>
        )}
      </Card>
    </div>
  );
};
```

### 6.2 APIクライアント

```typescript
// baseAccountApi.ts
import axios, { AxiosRequestConfig } from 'axios';
import {
  SearchBaseAccountsRequest,
  SearchBaseAccountsResponse,
  GetBaseAccountDetailResponse,
  CreateBaseAccountRequest,
  CreateBaseAccountResponse,
} from '../types/baseAccount';

const API_BASE_URL = '/api/v1';

const apiClient = axios.create({
  baseURL: API_BASE_URL,
  timeout: 30000,
  headers: {
    'Content-Type': 'application/json',
  },
});

apiClient.interceptors.request.use((config) => {
  const token = localStorage.getItem('auth_token');
  if (token) {
    config.headers.Authorization = `Bearer ${token}`;
  }
  config.headers['X-CSRF-Token'] = getCsrfToken();
  return config;
});

apiClient.interceptors.response.use(
  (response) => response.data,
  (error) => {
    if (error.response?.status === 401) {
      window.location.href = '/login';
    }
    return Promise.reject(error);
  }
);

export const searchBaseAccounts = async (
  params: SearchBaseAccountsRequest
): Promise<SearchBaseAccountsResponse> => {
  const queryString = new URLSearchParams();
  
  if (params.userId) queryString.append('userId', params.userId);
  if (params.email) queryString.append('email', params.email);
  if (params.userName) queryString.append('userName', params.userName);
  if (params.affiliation) queryString.append('affiliation', params.affiliation);
  if (params.permission) queryString.append('permission', params.permission);
  if (params.page) queryString.append('page', params.page.toString());
  if (params.pageSize) queryString.append('pageSize', params.pageSize.toString());
  if (params.sort) queryString.append('sort', params.sort);
  if (params.sortOrder) queryString.append('sortOrder', params.sortOrder);

  return apiClient.get(`/base-accounts/search?${queryString.toString()}`);
};

export const getBaseAccountDetail = async (
  userId: string
): Promise<GetBaseAccountDetailResponse> => {
  return apiClient.get(`/base-accounts/${userId}`);
};

export const createBaseAccount = async (
  request: CreateBaseAccountRequest
): Promise<CreateBaseAccountResponse> => {
  return apiClient.post('/base-accounts', request);
};

const getCsrfToken = () => {
  const metaTag = document.querySelector('meta[name="csrf-token"]');
  return metaTag ? metaTag.getAttribute('content') : '';
};
```

---

## 7. 后端服务设计

### 7.1 Controller

```java
@RestController
@RequestMapping("/api/v1/base-accounts")
@RequiredArgsConstructor
@Validated
public class BaseAccountController {

    private final BaseAccountService baseAccountService;
    private final AzureADService azureADService;

    @GetMapping("/search")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<SearchBaseAccountsResponse> searchAccounts(
            @Valid @ModelAttribute SearchBaseAccountsRequest request) {
        SearchBaseAccountsResponse response = baseAccountService.searchAccounts(request);
        return ResponseEntity.ok(response);
    }

    @GetMapping("/{userId}")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<GetBaseAccountDetailResponse> getAccountDetail(
            @PathVariable @NotBlank String userId) {
        GetBaseAccountDetailResponse response = baseAccountService.getAccountDetail(userId);
        return ResponseEntity.ok(response);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public ResponseEntity<CreateBaseAccountResponse> createAccount(
            @Valid @RequestBody CreateBaseAccountRequest request) {
        CreateBaseAccountResponse response = baseAccountService.createAccount(request);
        return ResponseEntity.ok(response);
    }
}
```

### 7.2 Service

```java
@Service
@RequiredArgsConstructor
@Slf4j
public class BaseAccountService {

    private final BaseAccountRepository baseAccountRepository;

    @Transactional(readOnly = true)
    public SearchBaseAccountsResponse searchAccounts(SearchBaseAccountsRequest request) {
        Specification<BaseAccount> spec = Specification.where(null);

        if (StringUtils.hasText(request.getUserId())) {
            spec = spec.and((root, query, cb) ->
                cb.like(cb.lower(root.get("userId")), "%" + request.getUserId().toLowerCase() + "%"));
        }

        if (StringUtils.hasText(request.getEmail())) {
            spec = spec.and((root, query, cb) ->
                cb.like(cb.lower(root.get("email")), "%" + request.getEmail().toLowerCase() + "%"));
        }

        if (StringUtils.hasText(request.getUserName())) {
            spec = spec.and((root, query, cb) ->
                cb.like(cb.lower(root.get("userName")), "%" + request.getUserName().toLowerCase() + "%"));
        }

        if (StringUtils.hasText(request.getAffiliation())) {
            spec = spec.and((root, query, cb) ->
                cb.like(cb.lower(root.get("affiliation")), "%" + request.getAffiliation().toLowerCase() + "%"));
        }

        if (request.getPermission() != null) {
            spec = spec.and((root, query, cb) ->
                cb.equal(root.get("permission"), PermissionType.valueOf(request.getPermission())));
        }

        Sort sort = Sort.by(Sort.Direction.ASC, "userId");
        if (StringUtils.hasText(request.getSort()) && request.getSortOrder() != null) {
            Sort.Direction direction = "asc".equalsIgnoreCase(request.getSortOrder())
                ? Sort.Direction.ASC : Sort.Direction.DESC;
            sort = Sort.by(direction, request.getSort());
        }

        Pageable pageable = PageRequest.of(
            request.getPage() != null ? request.getPage() - 1 : 0,
            request.getPageSize() != null ? request.getPageSize() : 20,
            sort);

        Page<BaseAccount> resultPage = baseAccountRepository.findAll(spec, pageable);

        List<BaseAccountInfo> accounts = resultPage.getContent().stream()
                .map(account -> BaseAccountInfo.builder()
                        .userId(account.getUserId())
                        .userName(account.getUserName())
                        .affiliation(account.getAffiliation())
                        .permission(account.getPermission().getValue())
                        .accountStatus(account.getAccountStatus().getValue())
                        .email(account.getEmail())
                        .createdAt(account.getCreatedAt().toString())
                        .lastLoginAt(account.getLastLoginAt()?.toString())
                        .build())
                .collect(Collectors.toList());

        SearchBaseAccountsSummary summary = calculateSummary(baseAccountRepository.findAll(spec));

        return SearchBaseAccountsResponse.builder()
                .success(true)
                .data(SearchBaseAccountsResponse.SearchResultData.builder()
                        .accounts(accounts)
                        .pagination(PaginationInfo.builder()
                                .currentPage(request.getPage() != null ? request.getPage() : 1)
                                .pageSize(request.getPageSize() != null ? request.getPageSize() : 20)
                                .totalItems((int) resultPage.getTotalElements())
                                .totalPages(resultPage.getTotalPages())
                                .build())
                        .summary(summary)
                        .build())
                .build();
    }

    private SearchBaseAccountsSummary calculateSummary(List<BaseAccount> accounts) {
        long adminCount = accounts.stream()
                .filter(a -> a.getPermission() == PermissionType.管理者)
                .count();
        long operatorCount = accounts.stream()
                .filter(a -> a.getPermission() != PermissionType.管理者)
                .count();

        return SearchBaseAccountsSummary.builder()
                .totalCount(accounts.size())
                .adminCount((int) adminCount)
                .operatorCount((int) operatorCount)
                .build();
    }
}
```

---

## 8. 数据库表设计

### 8.1 base_accountsテーブル

```sql
CREATE TABLE base_accounts (
    id BIGSERIAL PRIMARY KEY,
    user_id VARCHAR(50) NOT NULL UNIQUE COMMENT 'ユーザーID',
    user_name VARCHAR(200) NOT NULL COMMENT 'AzureADユーザー名',
    email VARCHAR(200) NOT NULL UNIQUE COMMENT 'メールアドレス',
    affiliation VARCHAR(200) COMMENT '所属ベース名',
    permission ENUM('管理者', '役割者', '配車担当者', '計上担当者') NOT NULL DEFAULT '役割者' COMMENT '権限区分',
    account_status ENUM('有効', '無効') NOT NULL DEFAULT '有効' COMMENT 'アカウント状態',
    azure_ad_id VARCHAR(100) COMMENT 'AzureADオブジェクトID',
    roles TEXT[] COMMENT '役割リスト',
    permissions TEXT[] COMMENT '権限リスト',
    last_login_at TIMESTAMP COMMENT '最終ログイン日時',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    
    INDEX idx_base_account_user_id (user_id),
    INDEX idx_base_account_email (email),
    INDEX idx_base_account_affiliation (affiliation),
    INDEX idx_base_account_permission (permission),
    INDEX idx_base_account_status (account_status),
    INDEX idx_base_account_azure_ad_id (azure_ad_id),
    INDEX idx_base_account_permission_status (permission, account_status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='ベースアカウントマスタテーブル';
```

### 8.2 base_account_audit_logテーブル

```sql
CREATE TABLE base_account_audit_log (
    id BIGSERIAL PRIMARY KEY,
    user_id VARCHAR(50) NOT NULL COMMENT '対象ユーザーID',
    action VARCHAR(50) NOT NULL COMMENT '操作区分',
    before_value TEXT COMMENT '変更前値',
    after_value TEXT COMMENT '変更後値',
    performed_by VARCHAR(100) NOT NULL COMMENT '実行者',
    performed_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    
    INDEX idx_audit_user_id (user_id),
    INDEX idx_audit_performed_at (performed_at),
    
    FOREIGN KEY (user_id) REFERENCES base_accounts(user_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='ベースアカウント操作監査ログテーブル';
```

---

## 9. 验收标准

### 9.1 功能验收

| 测试场景 | 预期结果 | 优先级 |
|----------|----------|--------|
| 页面初始加载 | 显示全账户列表，加载速度<2秒 | P0 |
| 用户ID搜索 | 支持部分匹配搜索 | P0 |
| 邮箱搜索 | 支持部分匹配搜索 | P0 |
| 用户名搜索 | 支持部分匹配搜索 | P0 |
| 所属搜索 | 支持部分匹配搜索 | P0 |
| 权限筛选 | 正确筛选指定权限账户 | P0 |
| 组合条件搜索 | 多条件AND逻辑正确 | P0 |
| 分页功能 | 正常翻页，每页大小切换 | P0 |
| 排序功能 | 各列点击排序正常 | P0 |
| 查看详情 | 跳转到账户详情页 | P0 |
| 新規追加 | 跳转到新增页面 | P0 |
| 空状态显示 | 无数据时显示友好提示 | P0 |
| 错误处理 | 网络错误时显示友好提示 | P0 |

### 9.2 视觉验收

| 项目 | 要求 |
|------|------|
| 布局 | 与设计稿一致 |
| 颜色 | 权限标签颜色正确 |
| 间距 | 符合UI规范 |
| 字体 | 大小、颜色正确 |

### 9.3 性能验收

| 指标 | 要求 | 测试方法 |
|------|------|----------|
| 页面加载 | <2秒 | Lighthouse |
| 搜索响应 | <1秒 | API监控 |
| 表格渲染 | <500ms | 浏览器DevTools |
| 分页切换 | <300ms | 用户体验测试 |

### 9.4 安全验收

| 检查项 | 要求 |
|--------|------|
| 权限控制 | 只有管理者能访问 |
| CSRF防护 | 所有POST请求带Token |
| XSS防护 | 用户输入正确转义 |
| SQL注入 | 使用参数化查询 |
| 日志审计 | 操作日志记录完整 |

---

## 10. 变更日志

| 日期 | 版本 | 变更内容 | 作者 |
|------|------|---------|------|
| 2026-02-XX| v1.0 | 初始版本 | - |

---

## 附录

### A.1 相关文档

| 文档 | 路径 | 说明 |
|------|------|------|
| 公共技术架构规范 | [\_公共技术架构规范.md](../_公共技术架构规范.md) | 前后端技术架构详细规范 |
| b15-ベースアカウント詳細 | [../b15-ベースアカウント詳細/05_设计规范.md](../b15-ベースアカウント詳細/05_设计规范.md) | 账户详情页面 |
| b11-輸送パートナーマスタ一覧 | [../b11-輸送パートナーマスタ一覧/05_设计規範.md](../b11-輸送パートナーマスタ一覧/05_设计规范.md) | 合作伙伴公司一览 |
| AzureAD連携仕様 | ../docs/azuread_integration.md | AzureAD連携详细说明 |

### A.2 関連画面遷移

| 画面 | 遷移元 | 遷移先 | トリガー | 説明 |
|------|--------|--------|----------|------|
| b14 | - | - | - | 初期表示画面 |
| b14 | - | b15 | ユーザーIDクリック | 詳細表示 |
| b14 | - | b14/new | ＋新規追加 | 新規アカウント登録 |

### A.3 API调用示例

```bash
# アカウント検索
curl -X GET "https://api.example.com/api/v1/base-accounts/search?userId=U&permission=管理者&page=1&pageSize=20" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
```

```bash
# アカウント詳細取得
curl -X GET "https://api.example.com/api/v1/base-accounts/U001" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
```

```bash
# アカウント新規作成
curl -X POST "https://api.example.com/api/v1/base-accounts" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..." \
  -H "Content-Type: application/json" \
  -d '{
    "userId": "U002",
    "userName": "山田太郎",
    "email": "taro.yamada@example.com",
    "affiliation": "東京ベース",
    "permission": "配車担当者",
    "accountStatus": "有効"
  }'
```

---

**文档编制**: -  
**编制时间**: 2026-02-XX 
**适用范围**: b14-ベースアカウントマスタ検索 基础账户搜索页面  
**交叉引用**: [\_公共技术架构规范.md](../_公共技术架构规范.md)
