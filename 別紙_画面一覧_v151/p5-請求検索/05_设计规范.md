# p5-請求検索 - 设计规范

## 1. 功能概述

### 1.1 文档信息

| 项目 | 内容 |
|------|------|
| **工作表名称** | p5-請求検索 |
| **界面编号** | p5 |
| **文档版本** | v1.0 |
| **最后更新** | 2026-02-XX|
| **文档状态** | ✅ 已发布 |

### 1.2 功能描述

p5-請求検索 是合作伙伴（パートナー）侧的账单搜索页面，提供账单信息查询、筛选、分页展示和详情查看功能。合作伙伴可以通过此页面查看和管理来自ベース（总部）的账单信息。

### 1.3 技术架构

本文档基于 **前后端分离架构**，详细技术规范请参考：

👉 **[\_公共技术架构规范.md](../_公共技术架构规范.md)**

| 层级 | 技术 |
|------|------|
| 前端 | React 18.x + TypeScript + Ant Design |
| 后端 | Spring Boot 3.x + Java 17 |
| 数据库 | PostgreSQL 15.x |

### 1.4 核心功能

| 功能 | 描述 | 优先级 |
|------|------|--------|
| 多条件搜索 | 按年月、账单ID、状态等搜索 | P0 |
| 分页展示 | 表格分页展示搜索结果 | P0 |
| 排序功能 | 各列排序支持 | P0 |
| 账单详情 | 查看账单详细信息 | P0 |
| 导出功能 | CSV/Excel格式导出 | P1 |
| 状态显示 | 彩色标签显示账单状态 | P0 |

### 1.5 适用范围

- 前端开发人员（搜索页面组件实现）
- 后端开发人员（账单API开发）
- 测试工程师（功能测试）
- 产品经理（功能确认）

---

## 2. 页面元素

### 2.1 元素清单

| 序号 | 元素名称 | 元素类型 | 功能说明 | 交互触发 |
|------|----------|----------|----------|----------|
| ⑴ | 検索条件エリア | フォーム | 年月/請求ID/ステータス | フォーム入力 |
| ⑵ | 年月セレクト | DatePicker.Month | 対象年月選択 | カレンダー選択 |
| ⑶ | 請求ID入力 | Input | 請求ID部分検索 | 文字入力 |
| ⑷ | ステータス選択 | Select | 請求ステータス選択 | プルダウン選択 |
| ⑸ | 検索ボタン | Primary Button | 検索実行 | クリック |
| ⑹ | エクスポートボタン | Default Button | データエクスポート | クリック |
| ⑺ | 結果テーブル | Table | 請求一覧表示 | 行クリック/ソート |
| ⑻ | 年月列 | Column | 請求年月 | 表示/ソート |
| ⑼ | 請求ID列 | Link Column | 請求ID | クリック→詳細 |
| ⑽ | 金額列 | Column | 請求金額 | 表示/ソート |
| ⑾ | ステータス列 | Tag Column | 請求ステータス | Tag表示 |
| ⑿ | 請求日列 | Column | 請求発行日 | 表示/ソート |
| ⒀ | 操作列 | Button Column | 詳細ボタン | クリック→詳細 |
| ⒁ | ページネーション | Pagination | ページング操作 | クリック |
| ⒂ | 合計表示 | Statistic | 検索結果合計 | 表示 |

### 2.2 検索条件详细说明

| フィールド | 要素タイプ | 制約 | データ形式 | デフォルト |
|-----------|-----------|------|-----------|----------|
| 年月 | DatePicker.Month | 過去12ヶ月 | YYYY-MM | 当月 |
| 請求ID | Input | 半角英数字50文字 | 部分一致 | 空 |
| ステータス | Select | 列挙値 | 未請求/請求済/支払済/キャンセル | 空 |

### 2.3 結果テーブル詳細説明

| カラム名 | 要素タイプ | 書式 | ソート | 固定 | 説明 |
|----------|-----------|------|--------|------|------|
| 年月 | Text | YYYY-MM | ○ | × | 請求対象月 |
| 請求ID | Link | 請求-XXXXXXXX | ○ | ○ | 詳細ページへ |
| 金額 | Number | ¥999,999 | ○ | × | 通貨書式 |
| ステータス | Tag | 色付き | ○ | × | 状態表示 |
| 請求日 | Date | YYYY/MM/DD | ○ | × | 発行日付 |
| 操作 | Button | 詳細 | × | ○ | 詳細表示 |

### 2.4 ステータス値詳細

| ステータス値 | 表示名 | タグ色 | 説明 |
|--------------|--------|--------|------|
| PENDING | 未請求 | 橙色 | 請求書が作成されたが未送付 |
| ISSUED | 請求済 | 青色 | 請求書が発行済み |
| PAID | 支払済 | 緑色 | 支払い完了 |
| CANCELLED | キャンセル | グレー | 請求がキャンセルされた |

---

## 3. 交互流程

### 3.1 ページロードフロー

| ステップ | アクション | システム応答 |
|----------|-----------|--------------|
| 1 | ユーザーがp5ページへ遷移 | ページ初期化 |
| 2 | デフォルト検索条件設定 | 年月:当月, 他:空 |
| 3 | 検索API自動実行 | GET /api/v1/partners/billings/search |
| 4 | APIレスポンス受信 | テーブルデータ設定 |
| 5 | テーブル描画 | ページネーション設定 |
| 6 | ページ表示完了 | ユーザー操作待機 |

### 3.2 検索操作フロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | 検索条件入力/選択 | リアルタイムバリデーション |
| 2 | 検索ボタンクリック | ローディング表示開始 |
| 3 | 検索パラメータ構築 | 年月:YYYY-MM, ID:部分一致 |
| 4 | APIリクエスト送信 | GET /api/v1/partners/billings/search |
| 5 | レスポンス受信 | テーブルデータ更新 |
| 6 | ページネーション更新 | 合計件数表示更新 |
| 7 | 検索完了 | ローディング表示終了 |

### 3.3 詳細表示フロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | 請求IDリンククリック | 詳細ページへ遷移 |
| 2 | 詳細ページロード | GET /api/v1/partners/billings/{id} |
| 3 | 詳細情報表示 | 請求詳細+明細表示 |
| 4 | 戻るボタンクリック | p5ページへ戻る |

### 3.4 ページングフロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | ページ番号クリック | ページ番号取得 |
| 2 | 検索API再実行 | page, pageSizeパラメータ追加 |
| 3 | テーブル更新 | 該当ページデータ表示 |
| 4 | スクロール位置調整 | テーブル上部へスクロール |

### 3.5 エクスポートフロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | エクスポートボタンクリック | 形式選択ダイアログ表示 |
| 2 | CSV/Excel選択 | 選択形式確認 |
| 3 | エクスポートAPI実行 | POST /api/v1/partners/billings/export |
| 4 | ファイルダウンロード | Content-Dispositionヘッダー処理 |
| 5 | ダウンロード完了 | 完了メッセージ表示 |

---

## 4. 界面规范

### 4.1 ページレイアウト

| 属性 | PC端値 | SP端値 | 説明 |
|------|--------|--------|------|
| **ページタイプ** | PC/SP対応 | レスポンシブ | ≪PC/SP≫両対応 |
| **レイアウト方式** | 上→下配置 | 垂直配置 | 検索→テーブル→ページング |
| **検索エリア** | 水平配置 | 垂直配置 | PC:3列, SP:縦1列 |

### 4.2 検索エリア仕様

| 属性 | 値 | 説明 |
|------|----|------|
| **ラベル幅** | 80px | 固定幅 |
| **入力欄幅** | 200px | PC端固定幅 |
| **ボタン間隔** | 16px | ボタン間余白 |
| **フォーム余白** | 24px | 検索エリア下部余白 |

### 4.3 テーブル仕様

| 属性 | 値 | 説明 |
|------|----|------|
| **行高** | 48px | 固定行高 |
| **フォントサイズ** | 14px | 標準テキスト |
| **縞模様** | 有り | 偶数行背景#fafafa |
| **ホバー効果** | 有り | 行ホバー背景#f5f5f5 |
| **ボーダー** | 1px #e8e8e8 | セル境界線 |
| **ヘッダー固定** | 有り | スクロール時ヘッダー固定 |

### 4.4 ステータスタグ仕様

| ステータス | 背景色 | 文字色 | ボーダー |
|-----------|--------|--------|--------|
| 未請求 | #fa8c16 | #ffffff | なし |
| 請求済 | #1890ff | #ffffff | なし |
| 支払済 | #52c41a | #ffffff | なし |
| キャンセル | #d9d9d9 | #000000 | なし |

### 4.5 ページネーション仕様

| 属性 | 値 | 説明 |
|------|----|------|
| **ページサイズ** | 10/20/50/100 | 選択可能 |
| **表示位置** | 右下 | テーブル下中央 |
| **総件数表示** | 有り | "1-20 / 100件" 形式 |

---

## 5. API接口设计

### 5.1 請求検索API

```typescript
// GET /api/v1/partners/billings/search
interface SearchBillingRequest {
  yearMonth?: string;          // YYYY-MM格式
  billingId?: string;          // 部分匹配
  status?: BillingStatus;      // 状态筛选
  page?: number;               // 页码，从1开始
  pageSize?: number;           // 每页数量，默认20
  sort?: string;               // 排序字段
  sortOrder?: 'asc' | 'desc';  // 排序方向
}

interface SearchBillingResponse {
  success: boolean;
  data: {
    billings: BillingItem[];
    pagination: {
      currentPage: number;
      pageSize: number;
      totalItems: number;
      totalPages: number;
    };
    summary: {
      totalCount: number;       // 总记录数
      totalAmount: number;      // 总金额
      pendingCount: number;     // 未請求数量
      issuedCount: number;      // 請求済数量
      paidCount: number;        // 支払済数量
    };
  };
  error?: {
    code: string;
    message: string;
    details?: Record<string, unknown>;
  };
}

type BillingStatus = 'PENDING' | 'ISSUED' | 'PAID' | 'CANCELLED';

interface BillingItem {
  billingId: string;           // 請求ID
  billingYearMonth: string;    // 請求年月 YYYY-MM
  amount: number;              // 金額
  status: BillingStatus;       // ステータス
  issueDate: string;           // 請求日 YYYY-MM-DD
  dueDate?: string;            // 支払期限
  partnerId: string;           // パートナーID
  createdAt: string;           // 作成日時
}
```

### 5.2 請求詳細API

```typescript
// GET /api/v1/partners/billings/{billingId}
interface GetBillingDetailResponse {
  success: boolean;
  data: {
    billingId: string;
    billingYearMonth: string;
    amount: number;
    status: BillingStatus;
    issueDate: string;
    dueDate?: string;
    paidDate?: string;
    partnerId: string;
    partnerName: string;
    baseName: string;
    details: BillingDetail[];
    attachments?: Attachment[];
    createdAt: string;
    updatedAt: string;
  };
}

interface BillingDetail {
  detailId: string;
  lineCode: string;            // 線便コード
  lineName: string;            // 線便名
  dailyRate: number;           // 日別傭車費
  days: number;                // 日数
  amount: number;              // 金額（dailyRate × days）
  description?: string;        // 説明
}

interface Attachment {
  fileId: string;
  fileName: string;
  fileType: string;
  fileSize: number;
  uploadedAt: string;
}
```

### 5.3 エクスポートAPI

```typescript
// POST /api/v1/partners/billings/export
interface ExportBillingRequest {
  searchConditions: SearchBillingRequest;
  format: 'csv' | 'excel';        // 导出格式
  includeDetails?: boolean;       // 是否包含明细
  language?: 'ja' | 'zh' | 'en';  // 导出语言
}

interface ExportBillingResponse {
  success: boolean;
  data: {
    exportId: string;
    fileName: string;
    fileSize: number;
    downloadUrl: string;
    expiresAt: string;           // 下载链接有效期
  };
  message: string;
}
```

### 5.4 APIエラーレスポンス

```typescript
interface ApiErrorResponse {
  success: false;
  error: {
    code: ErrorCode;
    message: string;
    details?: Record<string, unknown>;
  };
}

type ErrorCode =
  | 'INVALID_PARAMETER'
  | 'PARTNER_NOT_FOUND'
  | 'BILLING_NOT_FOUND'
  | 'PERMISSION_DENIED'
  | 'EXPORT_FAILED'
  | 'RATE_LIMIT_EXCEEDED'
  | 'INTERNAL_ERROR';
```

---

## 6. 前端实现

### 6.1 メインコンポーネント

```typescript
// BillingSearchPage.tsx
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import {
  Form,
  DatePicker,
  Input,
  Select,
  Button,
  Table,
  Tag,
  Pagination,
  Space,
  message,
  Card,
  Statistic,
} from 'antd';
import { SearchOutlined, ExportOutlined, EyeOutlined } from '@ant-design/icons';
import dayjs from 'dayjs';
import { BillingItem, SearchBillingRequest } from '../../types/billing';
import { searchBillings, exportBillings } from '../../api/billing';
import { StatusTag } from '../../components/StatusTag';
import styles from './BillingSearchPage.module.css';

const { RangePicker } = DatePicker;

export const BillingSearchPage: React.FC = () => {
  const [form] = Form.useForm();
  const navigate = useNavigate();
  const [data, setData] = useState<BillingItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [exporting, setExporting] = useState(false);
  const [pagination, setPagination] = useState({
    current: 1,
    pageSize: 20,
    total: 0,
  });
  const [summary, setSummary] = useState({
    totalAmount: 0,
    totalCount: 0,
  });

  // 列定义
  const columns = [
    {
      title: '年月',
      dataIndex: 'billingYearMonth',
      key: 'billingYearMonth',
      width: 100,
      sorter: true,
      render: (text: string) => (
        <span className={styles.yearMonth}>{text}</span>
      ),
    },
    {
      title: '請求ID',
      dataIndex: 'billingId',
      key: 'billingId',
      width: 180,
      sorter: true,
      render: (text: string, record: BillingItem) => (
        <Button
          type="link"
          onClick={() => handleViewDetail(record.billingId)}
          className={styles.billingIdLink}
        >
          {text}
        </Button>
      ),
    },
    {
      title: '金額',
      dataIndex: 'amount',
      key: 'amount',
      width: 120,
      sorter: true,
      align: 'right' as const,
      render: (amount: number) => (
        <span className={styles.amount}>
          ¥{amount.toLocaleString('ja-JP')}
        </span>
      ),
    },
    {
      title: 'ステータス',
      dataIndex: 'status',
      key: 'status',
      width: 100,
      sorter: true,
      render: (status: string) => <StatusTag status={status} />,
    },
    {
      title: '請求日',
      dataIndex: 'issueDate',
      key: 'issueDate',
      width: 120,
      sorter: true,
      render: (date: string) => dayjs(date).format('YYYY/MM/DD'),
    },
    {
      title: '操作',
      key: 'action',
      width: 100,
      fixed: 'right' as const,
      render: (_: any, record: BillingItem) => (
        <Button
          type="text"
          icon={<EyeOutlined />}
          onClick={() => handleViewDetail(record.billingId)}
          size="small"
        >
          詳細
        </Button>
      ),
    },
  ];

  // 初始搜索
  useEffect(() => {
    handleSearch({ yearMonth: dayjs().format('YYYY-MM') });
  }, []);

  // 搜索处理
  const handleSearch = async (values: any) => {
    setLoading(true);
    try {
      const searchParams: SearchBillingRequest = {
        ...values,
        page: pagination.current,
        pageSize: pagination.pageSize,
      };

      const response = await searchBillings(searchParams);
      if (response.success) {
        setData(response.data.billings);
        setPagination(prev => ({
          ...prev,
          total: response.data.pagination.totalItems,
        }));
        setSummary({
          totalAmount: response.data.summary.totalAmount,
          totalCount: response.data.summary.totalCount,
        });
      } else {
        message.error(response.error?.message || '検索に失敗しました');
      }
    } catch (error) {
      message.error('ネットワークエラーが発生しました');
      console.error('Search error:', error);
    } finally {
      setLoading(false);
    }
  };

  // 页面变化处理
  const handlePageChange = (page: number, pageSize?: number) => {
    setPagination(prev => ({
      ...prev,
      current: page,
      pageSize: pageSize || prev.pageSize,
    }));
    
    // 重新搜索
    form.validateFields().then(values => {
      handleSearch({
        ...values,
        page,
        pageSize: pageSize || pagination.pageSize,
      });
    });
  };

  // 查看详情
  const handleViewDetail = (billingId: string) => {
    navigate(`/p5/detail/${billingId}`);
  };

  // 导出处理
  const handleExport = async () => {
    setExporting(true);
    try {
      const values = await form.validateFields();
      const searchParams: SearchBillingRequest = {
        ...values,
        page: 1,
        pageSize: 10000, // 导出所有数据
      };

      const response = await exportBillings({
        searchConditions: searchParams,
        format: 'excel',
        includeDetails: true,
      });

      if (response.success) {
        // 下载文件
        const link = document.createElement('a');
        link.href = response.data.downloadUrl;
        link.download = response.data.fileName;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);

        message.success('エクスポートが完了しました');
      } else {
        message.error(response.error?.message || 'エクスポートに失敗しました');
      }
    } catch (error) {
      message.error('エクスポート中にエラーが発生しました');
      console.error('Export error:', error);
    } finally {
      setExporting(false);
    }
  };

  // 重置搜索
  const handleReset = () => {
    form.resetFields();
    setPagination({
      current: 1,
      pageSize: 20,
      total: 0,
    });
    setData([]);
    setSummary({ totalAmount: 0, totalCount: 0 });
  };

  return (
    <div className={styles.container}>
      <Card title="請求検索" className={styles.searchCard}>
        <Form
          form={form}
          layout="inline"
          onFinish={handleSearch}
          initialValues={{
            yearMonth: dayjs().format('YYYY-MM'),
          }}
        >
          <Form.Item
            name="yearMonth"
            label="年月"
            className={styles.formItem}
          >
            <DatePicker
              picker="month"
              format="YYYY-MM"
              allowClear={false}
              className={styles.datePicker}
            />
          </Form.Item>

          <Form.Item
            name="billingId"
            label="請求ID"
            className={styles.formItem}
          >
            <Input
              placeholder="請求IDを入力"
              allowClear
              maxLength={50}
              className={styles.input}
            />
          </Form.Item>

          <Form.Item
            name="status"
            label="ステータス"
            className={styles.formItem}
          >
            <Select
              placeholder="ステータスを選択"
              allowClear
              style={{ width: 120 }}
              options={[
                { value: 'PENDING', label: '未請求' },
                { value: 'ISSUED', label: '請求済' },
                { value: 'PAID', label: '支払済' },
                { value: 'CANCELLED', label: 'キャンセル' },
              ]}
            />
          </Form.Item>

          <Form.Item className={styles.formItem}>
            <Space>
              <Button
                type="primary"
                htmlType="submit"
                icon={<SearchOutlined />}
                loading={loading}
              >
                検索
              </Button>
              <Button onClick={handleReset}>
                リセット
              </Button>
              <Button
                icon={<ExportOutlined />}
                onClick={handleExport}
                loading={exporting}
              >
                エクスポート
              </Button>
            </Space>
          </Form.Item>
        </Form>
      </Card>

      {summary.totalCount > 0 && (
        <div className={styles.summary}>
          <Statistic
            title="合計件数"
            value={summary.totalCount}
            suffix="件"
          />
          <Statistic
            title="合計金額"
            value={summary.totalAmount}
            prefix="¥"
            valueStyle={{ color: '#3f8600' }}
          />
        </div>
      )}

      <Card className={styles.resultCard}>
        <Table
          columns={columns}
          dataSource={data}
          rowKey="billingId"
          loading={loading}
          pagination={false}
          scroll={{ x: 800 }}
          locale={{
            emptyText: '検索条件に一致する請求がありません',
          }}
        />

        {pagination.total > 0 && (
          <div className={styles.pagination}>
            <Pagination
              current={pagination.current}
              pageSize={pagination.pageSize}
              total={pagination.total}
              onChange={handlePageChange}
              onShowSizeChange={handlePageChange}
              showSizeChanger
              showQuickJumper
              showTotal={(total, range) => (
                `${range[0]}-${range[1]} / ${total}件`
              )}
              pageSizeOptions={['10', '20', '50', '100']}
            />
          </div>
        )}
      </Card>
    </div>
  );
};
```

### 6.2 ステータスタグコンポーネント

```typescript
// StatusTag.tsx
import React from 'react';
import { Tag } from 'antd';
import { BillingStatus } from '../../types/billing';

interface StatusTagProps {
  status: BillingStatus;
  size?: 'default' | 'small';
}

export const StatusTag: React.FC<StatusTagProps> = ({ status, size = 'default' }) => {
  const statusConfig = {
    PENDING: {
      color: 'orange',
      text: '未請求',
      description: '請求書が作成されたが未送付',
    },
    ISSUED: {
      color: 'blue',
      text: '請求済',
      description: '請求書が発行済み',
    },
    PAID: {
      color: 'green',
      text: '支払済',
      description: '支払い完了',
    },
    CANCELLED: {
      color: 'default',
      text: 'キャンセル',
      description: '請求がキャンセルされた',
    },
  };

  const config = statusConfig[status] || {
    color: 'default',
    text: status,
    description: '',
  };

  return (
    <Tag
      color={config.color}
      style={{
        fontSize: size === 'small' ? '12px' : '14px',
        padding: size === 'small' ? '2px 8px' : '4px 12px',
        borderRadius: '4px',
      }}
      title={config.description}
    >
      {config.text}
    </Tag>
  );
};
```

### 6.3 APIクライアント

```typescript
// billingApi.ts
import axios, { AxiosRequestConfig } from 'axios';
import {
  SearchBillingRequest,
  SearchBillingResponse,
  ExportBillingRequest,
  ExportBillingResponse,
  GetBillingDetailResponse,
} from '../types/billing';

const API_BASE_URL = '/api/v1';

const apiClient = axios.create({
  baseURL: API_BASE_URL,
  timeout: 30000,
  headers: {
    'Content-Type': 'application/json',
  },
});

// 请求拦截器（添加token）
apiClient.interceptors.request.use((config) => {
  const token = localStorage.getItem('auth_token');
  if (token) {
    config.headers.Authorization = `Bearer ${token}`;
  }
  config.headers['X-CSRF-Token'] = getCsrfToken();
  return config;
});

// 响应拦截器（错误处理）
apiClient.interceptors.response.use(
  (response) => response.data,
  (error) => {
    if (error.response?.status === 401) {
      // 认证失败，跳转到登录页
      window.location.href = '/login';
    }
    return Promise.reject(error);
  }
);

export const searchBillings = async (
  params: SearchBillingRequest
): Promise<SearchBillingResponse> => {
  const queryString = new URLSearchParams();
  
  if (params.yearMonth) queryString.append('yearMonth', params.yearMonth);
  if (params.billingId) queryString.append('billingId', params.billingId);
  if (params.status) queryString.append('status', params.status);
  if (params.page) queryString.append('page', params.page.toString());
  if (params.pageSize) queryString.append('pageSize', params.pageSize.toString());
  if (params.sort) queryString.append('sort', params.sort);
  if (params.sortOrder) queryString.append('sortOrder', params.sortOrder);

  return apiClient.get(`/partners/billings/search?${queryString.toString()}`);
};

export const getBillingDetail = async (
  billingId: string
): Promise<GetBillingDetailResponse> => {
  return apiClient.get(`/partners/billings/${billingId}`);
};

export const exportBillings = async (
  request: ExportBillingRequest
): Promise<ExportBillingResponse> => {
  return apiClient.post('/partners/billings/export', request);
};

// CSRF Token获取
const getCsrfToken = () => {
  const metaTag = document.querySelector('meta[name="csrf-token"]');
  return metaTag ? metaTag.getAttribute('content') : '';
};
```

---

## 7. 后端服务设计

### 7.1 Controller

```java
@RestController
@RequestMapping("/api/v1/partners/billings")
@RequiredArgsConstructor
@Validated
public class PartnerBillingController {

    private final PartnerBillingService billingService;
    private final BillingExportService exportService;

    @GetMapping("/search")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<SearchBillingResponse> searchBillings(
            @Valid @ModelAttribute SearchBillingRequest request,
            @AuthenticationPrincipal User user) {
        SearchBillingResponse response = billingService.searchBillings(
            request, user.getPartnerId());
        return ResponseEntity.ok(response);
    }

    @GetMapping("/{billingId}")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<GetBillingDetailResponse> getBillingDetail(
            @PathVariable @NotBlank String billingId,
            @AuthenticationPrincipal User user) {
        GetBillingDetailResponse response = billingService.getBillingDetail(
            billingId, user.getPartnerId());
        return ResponseEntity.ok(response);
    }

    @PostMapping("/export")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<Resource> exportBillings(
            @Valid @RequestBody ExportBillingRequest request,
            @AuthenticationPrincipal User user) throws IOException {
        ExportResult exportResult = exportService.exportBillings(
            request, user.getPartnerId());

        ByteArrayResource resource = new ByteArrayResource(exportResult.getData());

        return ResponseEntity.ok()
            .contentType(MediaType.parseMediaType(exportResult.getContentType()))
            .header(HttpHeaders.CONTENT_DISPOSITION,
                    "attachment; filename=\"" + exportResult.getFileName() + "\"")
            .body(resource);
    }

    @ExceptionHandler(ValidationException.class)
    public ResponseEntity<ApiErrorResponse> handleValidationException(
            ValidationException ex) {
        return ResponseEntity.badRequest()
            .body(ApiErrorResponse.builder()
                .success(false)
                .error(ApiError.builder()
                    .code("INVALID_PARAMETER")
                    .message(ex.getMessage())
                    .build())
                .build());
    }

    @ExceptionHandler(PermissionDeniedException.class)
    public ResponseEntity<ApiErrorResponse> handlePermissionDeniedException(
            PermissionDeniedException ex) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
            .body(ApiErrorResponse.builder()
                .success(false)
                .error(ApiError.builder()
                    .code("PERMISSION_DENIED")
                    .message(ex.getMessage())
                    .build())
                .build());
    }
}
```

### 7.2 Service

```java
@Service
@RequiredArgsConstructor
@Slf4j
public class PartnerBillingService {

    private final BillingRepository billingRepository;
    private final BillingDetailRepository detailRepository;
    private final PartnerRepository partnerRepository;

    @Transactional(readOnly = true)
    public SearchBillingResponse searchBillings(
            SearchBillingRequest request, String partnerId) {
        // 验证合作伙伴存在性
        Partner partner = partnerRepository.findById(partnerId)
            .orElseThrow(() -> new PartnerNotFoundException(partnerId));

        // 构建查询条件
        Specification<Billing> spec = Specification.where(
            BillingSpecification.hasPartnerId(partnerId));

        if (StringUtils.hasText(request.getYearMonth())) {
            spec = spec.and(BillingSpecification.hasYearMonth(
                request.getYearMonth()));
        }

        if (StringUtils.hasText(request.getBillingId())) {
            spec = spec.and(BillingSpecification.billingIdContains(
                request.getBillingId()));
        }

        if (request.getStatus() != null) {
            spec = spec.and(BillingSpecification.hasStatus(
                request.getStatus()));
        }

        // 分页查询
        Pageable pageable = PageRequest.of(
            request.getPage() != null ? request.getPage() - 1 : 0,
            request.getPageSize() != null ? request.getPageSize() : 20,
            buildSort(request.getSort(), request.getSortOrder()));

        Page<Billing> billingPage = billingRepository.findAll(spec, pageable);

        // 统计数据
        List<Billing> allBillings = billingRepository.findAll(spec);
        BillingSummary summary = calculateSummary(allBillings);

        // 转换为DTO
        List<BillingDTO> billingDTOs = billingPage.getContent().stream()
            .map(BillingDTO::from)
            .collect(Collectors.toList());

        return SearchBillingResponse.builder()
            .success(true)
            .data(SearchBillingData.builder()
                .billings(billingDTOs)
                .pagination(PaginationInfo.builder()
                    .currentPage(billingPage.getNumber() + 1)
                    .pageSize(billingPage.getSize())
                    .totalItems(billingPage.getTotalElements())
                    .totalPages(billingPage.getTotalPages())
                    .build())
                .summary(summary)
                .build())
            .build();
    }

    @Transactional(readOnly = true)
    public GetBillingDetailResponse getBillingDetail(
            String billingId, String partnerId) {
        Billing billing = billingRepository.findById(billingId)
            .orElseThrow(() -> new BillingNotFoundException(billingId));

        // 验证权限
        validateBillingOwnership(billing, partnerId);

        // 获取明细
        List<BillingDetail> details = detailRepository.findByBillingId(billingId);

        // 获取合作伙伴信息
        Partner partner = partnerRepository.findById(partnerId)
            .orElseThrow(() -> new PartnerNotFoundException(partnerId));

        return GetBillingDetailResponse.builder()
            .success(true)
            .data(BillingDetailDTO.from(billing, details, partner))
            .build();
    }

    private BillingSummary calculateSummary(List<Billing> billings) {
        long pendingCount = billings.stream()
            .filter(b -> b.getStatus() == BillingStatus.PENDING)
            .count();
        long issuedCount = billings.stream()
            .filter(b -> b.getStatus() == BillingStatus.ISSUED)
            .count();
        long paidCount = billings.stream()
            .filter(b -> b.getStatus() == BillingStatus.PAID)
            .count();

        BigDecimal totalAmount = billings.stream()
            .map(Billing::getAmount)
            .reduce(BigDecimal.ZERO, BigDecimal::add);

        return BillingSummary.builder()
            .totalCount(billings.size())
            .totalAmount(totalAmount)
            .pendingCount(pendingCount)
            .issuedCount(issuedCount)
            .paidCount(paidCount)
            .build();
    }

    private Sort buildSort(String sortField, String sortOrder) {
        if (sortField == null) {
            return Sort.by(Sort.Order.desc("issueDate"));
        }

        Sort.Direction direction = "desc".equalsIgnoreCase(sortOrder)
            ? Sort.Direction.DESC : Sort.Direction.ASC;

        return Sort.by(new Sort.Order(direction, sortField));
    }

    private void validateBillingOwnership(Billing billing, String partnerId) {
        if (!billing.getPartnerId().equals(partnerId)) {
            log.warn("パートナー{}が請求{}へのアクセスを試みましたが拒否されました",
                partnerId, billing.getId());
            throw new PermissionDeniedException(
                "この請求へのアクセス権限がありません");
        }
    }
}
```

---

## 8. 数据库表设计

### 8.1 billingsテーブル

```sql
CREATE TABLE billings (
    id VARCHAR(50) PRIMARY KEY,
    billing_id VARCHAR(50) UNIQUE NOT NULL COMMENT '請求ID',
    billing_year_month VARCHAR(7) NOT NULL COMMENT '請求年月 (YYYY-MM)',
    amount DECIMAL(12,2) NOT NULL COMMENT '請求金額',
    status ENUM('PENDING', 'ISSUED', 'PAID', 'CANCELLED') NOT NULL DEFAULT 'PENDING',
    issue_date DATE NOT NULL COMMENT '請求日',
    due_date DATE COMMENT '支払期限',
    paid_date DATE COMMENT '支払日',
    partner_id VARCHAR(50) NOT NULL COMMENT 'パートナーID',
    base_name VARCHAR(200) NOT NULL COMMENT 'ベース名',
    memo TEXT COMMENT '備考',
    
    -- 审计字段
    created_by VARCHAR(100) COMMENT '作成者',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_by VARCHAR(100) COMMENT '更新者',
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    
    -- 索引
    INDEX idx_billing_partner_id (partner_id),
    INDEX idx_billing_year_month (billing_year_month),
    INDEX idx_billing_status (status),
    INDEX idx_billing_issue_date (issue_date),
    INDEX idx_billing_partner_status (partner_id, status),
    
    -- 外键约束
    FOREIGN KEY (partner_id) REFERENCES partners(partner_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='請求情報テーブル';
```

### 8.2 billing_detailsテーブル

```sql
CREATE TABLE billing_details (
    id VARCHAR(50) PRIMARY KEY,
    billing_id VARCHAR(50) NOT NULL COMMENT '請求ID',
    line_code VARCHAR(20) NOT NULL COMMENT '線便コード',
    line_name VARCHAR(100) NOT NULL COMMENT '線便名',
    daily_rate DECIMAL(10,2) NOT NULL COMMENT '日別傭車費',
    days INT NOT NULL COMMENT '日数',
    amount DECIMAL(10,2) NOT NULL COMMENT '金額 (daily_rate × days)',
    description TEXT COMMENT '説明',
    sort_order INT DEFAULT 0 COMMENT '表示順序',
    
    -- 审计字段
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    
    -- 索引
    INDEX idx_detail_billing_id (billing_id),
    INDEX idx_detail_line_code (line_code),
    
    -- 外键约束
    FOREIGN KEY (billing_id) REFERENCES billings(id) ON DELETE CASCADE,
    
    -- 检查约束
    CONSTRAINT chk_days_positive CHECK (days > 0),
    CONSTRAINT chk_amount_correct CHECK (amount = daily_rate * days)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='請求明細テーブル';
```

### 8.3 billing_attachmentsテーブル

```sql
CREATE TABLE billing_attachments (
    id VARCHAR(50) PRIMARY KEY,
    billing_id VARCHAR(50) NOT NULL COMMENT '請求ID',
    file_name VARCHAR(255) NOT NULL COMMENT 'ファイル名',
    file_type VARCHAR(50) NOT NULL COMMENT 'ファイルタイプ',
    file_size BIGINT NOT NULL COMMENT 'ファイルサイズ (bytes)',
    file_path VARCHAR(500) NOT NULL COMMENT 'ファイルパス',
    uploaded_by VARCHAR(100) NOT NULL COMMENT 'アップロード者',
    uploaded_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    
    -- 索引
    INDEX idx_attachment_billing_id (billing_id),
    
    -- 外键约束
    FOREIGN KEY (billing_id) REFERENCES billings(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='請求添付ファイルテーブル';
```

---

## 9. 验收标准

### 9.1 功能验收

| 测试场景 | 预期结果 | 优先级 |
|----------|----------|--------|
| 页面初始加载 | 显示当月账单，加载速度<2秒 | P0 |
| 年月条件搜索 | 正确筛选指定年月账单 | P0 |
| 账单ID搜索 | 支持部分匹配搜索 | P0 |
| 状态筛选 | 正确筛选指定状态账单 | P0 |
| 组合条件搜索 | 多条件AND逻辑正确 | P0 |
| 分页功能 | 正常翻页，每页大小切换 | P0 |
| 排序功能 | 各列点击排序正常 | P0 |
| 查看详情 | 跳转到账单详情页 | P0 |
| 导出功能 | 生成正确的Excel/CSV文件 | P1 |
| 空状态显示 | 无数据时显示友好提示 | P0 |
| 错误处理 | 网络错误时显示友好提示 | P0 |

### 9.2 视觉验收

| 项目 | 要求 |
|------|------|
| 布局 | 与设计稿一致 |
| 颜色 | 状态标签颜色正确 |
| 间距 | 符合UI规范 |
| 响应式 | 适配PC/SP |
| 字体 | 大小、颜色正确 |

### 9.3 性能验收

| 指标 | 要求 | 测试方法 |
|------|------|----------|
| 页面加载 | <2秒 | Lighthouse |
| 搜索响应 | <1秒 | API监控 |
| 表格渲染 | <500ms | 浏览器DevTools |
| 分页切换 | <300ms | 用户体验测试 |

### 9.4 安全验收

| 检查项 | 要求 |
|--------|------|
| 权限控制 | 只能查看自己的账单 |
| CSRF防护 | 所有POST请求带Token |
| XSS防护 | 用户输入正确转义 |
| SQL注入 | 使用参数化查询 |
| 日志审计 | 操作日志记录完整 |

---

## 10. 变更日志

| 日期 | 版本 | 变更内容 | 作者 |
|------|------|---------|------|
| 2026-02-XX| v1.0 | 初始版本 | - |

---

## 附录

### A.1 相关文档

| 文档 | 路径 | 说明 |
|------|------|------|
| 公共技术架构规范 | [\_公共技术架构规范.md](../_公共技术架构规范.md) | 前后端技术架构详细规范 |
| b5-実績詳細 | [../b5-実績詳細/05_设计规范.md](../b5-実績詳細/05_设计规范.md) | Base侧业绩详情设计规范 |
| p3-発注詳細 | [../p3-発注詳細/05_设计规范.md](../p3-発注詳細/05_设计规范.md) | 合作伙伴订单详情设计规范 |

### A.2 API调用示例

```bash
# 搜索请求示例
curl -X GET "https://api.example.com/api/v1/partners/billings/search?yearMonth=2025-01&status=PAID&page=1&pageSize=20" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
```

```bash
# 导出请求示例
curl -X POST "https://api.example.com/api/v1/partners/billings/export" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..." \
  -H "Content-Type: application/json" \
  -d '{
    "searchConditions": {
      "yearMonth": "2025-01",
      "status": "PAID"
    },
    "format": "excel",
    "includeDetails": true
  }'
```

---

**文档编制**: -  
**编制时间**: 2026-02-XX 
**适用范围**: p5-請求検索 合作伙伴账单搜索页面  
**交叉引用**: [\_公共技术架构规范.md](../_公共技术架构规范.md)
