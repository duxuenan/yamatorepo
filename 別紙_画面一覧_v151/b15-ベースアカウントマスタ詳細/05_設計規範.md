# b15-ベースアカウントマスタ詳細 - 设计规范

## 1. 功能概述

### 1.1 文档信息

| 项目 | 内容 |
|------|------|
| **工作表名称** | b15-ベースアカウントマスタ詳細 |
| **界面编号** | b15 |
| **对应Base界面** | b14-ベースアカウントマスタ検索 |
| **文档版本** | v1.0 |
| **最后更新** | 2026-02-XX|
| **文档状态** | ✅ 已发布 |

### 1.2 功能描述

b15-ベースアカウントマスタ詳細是ベース（总部）侧的基础账户详情管理页面。管理者可以通过此页面：

- 查看用户的只读信息（ユーザーID、メール、氏名）
- 管理用户所属（所属ベース選択）
- 管理用户权限（管理者/役割者/配車担当者/計上担当者）
- 查看权限矩阵（権限マトリクス）
- 管理アカウント有効・無効状态
- 権限変更履歴の確認

### 1.3 技术架构

本文档基于 **前后端分离架构**，详细技术规范请参考：

👉 **[\_公共技术架构规范.md](../_公共技术架构规范.md)**

| 层级 | 技术 |
|------|------|
| 前端 | React 18.x + TypeScript + Ant Design |
| 后端 | Spring Boot 3.x + Java 17 |
| 数据库 | PostgreSQL 15.x |

### 1.4 核心功能

| 功能 | 描述 | 优先级 |
|------|------|--------|
| ユーザー情報表示 | 显示用户只读信息 | P0 |
| 所属設定 | 所属ベース的选择和编辑 | P0 |
| 権限設定 | 権限区分の选择和编辑 | P0 |
| 権限マトリクス表示 | 权限矩阵的可视化展示 | P0 |
| アカウント状態管理 | 有効/無効状态切换 | P0 |
| 編集モード切替 | 查看/编辑模式的切换 | P0 |

### 1.5 适用范围

- 前端开发人员（详情页面组件实现）
- 后端开发人员（账户管理API开发）
- 测试工程师（功能测试）

---

## 2. 页面元素

### 2.1 元素清单

| 序号 | 元素名称 | 元素类型 | 功能说明 | 交互触发 |
|------|----------|----------|----------|----------|
| ⑴ | ページタイトル | Text | 页面标题显示 | 表示 |
| ⑵ | 編集モード切替 | Button | 编辑/查看模式切换 | クリック |
| ⑶ | ユーザー情報セクション | Card | 用户信息只读区域 | 表示 |
| ⑷ | ユーザーID | Text | 用户ID显示 | 表示 |
| ⑸ | メール | Text | 邮箱地址显示 | 表示 |
| ⑹ | 氏名 | Text | 用户姓名显示 | 表示 |
| ⑺ | 所属セクション | Card | 所属设置区域 | 表示 |
| ⑻ | 所属ベース | Select/Text | 所属ベース选择 | プルダウン選択 |
| ⑼ | 権限セクション | Card | 权限设置区域 | 表示 |
| ⑽ | 権限区分 | Radio | 権限区分选择 | ラジオ選択 |
| ⑾ | 権限マトリクス | Table | 权限矩阵显示 | 表示 |
| ⑿ | 権限マトリクス見出し | Text | 権限区分标题 | 表示 |
| ⒀ | 権限付与列 | Tag | 権限付与权限 | Tag表示 |
| ⒁ | 承認列 | Tag | 承認権限 | Tag表示 |
| ⒂ | マスタ管理列 | Tag | マスタ管理権限 | Tag表示 |
| ⒃ | 発注列 | Tag | 発注権限 | Tag表示 |
| ⒄ | 計上列 | Tag | 計上権限 | Tag表示 |
| ⒅ | アカウント状態セクション | Card | 账户状态区域 | 表示 |
| ⒆ | 有効/無効切替 | Switch | 状态切换开关 | スイッチ |
| ⒇ | 状態説明 | Text | 状态说明文字 | 表示 |
| (21) | 戻るボタン | Button | 返回上一页 | クリック |
| (22) | 保存ボタン | Primary Button | 保存设置 | クリック |
| (23) | キャンセルボタン | Default Button | 取消编辑 | クリック |

### 2.2 フィールド詳細説明

| フィールド | 要素タイプ | 制約 | データ形式 | デフォルト | 編集可否 |
|-----------|-----------|------|-----------|----------|----------|
| ユーザーID | Text | 必須 | 文字列 | - | 閲覧のみ |
| メール | Text | 必須,email形式 | 文字列 | - | 閲覧のみ |
| 氏名 | Text | 必須 | 文字列 | - | 閲覧のみ |
| 所属ベース | Select | 必須 | リストから選択 | - | 編集可 |
| 権限区分 | Radio | 必須 | 列挙値 | 役割者 | 編集可 |
| アカウント有効・無効 | Switch | 必須 | 有効/無効 | 有効 | 編集可 |

### 2.3 権限区分値詳細

| 権限値 | 表示名 | タグ色 | 説明 |
|--------|--------|--------|------|
| 管理者 | 管理者 | 赤色 | 全権限持有者 |
| 役割者 | 役割者 | 青色 | 一般役割者 |
| 配車担当者 | 配車担当者 | 緑色 | 配車業務担当者 |
| 計上担当者 | 計上担当者 | 橙色 | 計上業務担当者 |

### 2.4 権限マトリクス詳細

| 権限区分 | 権限付与 | 承認 | マスタ管理 | 発注 | 計上 |
|----------|----------|------|------------|------|------|
| 管理者 | 編集可 | 編集可 | 編集可 | 閲覧のみ | 閲覧のみ |
| 役割者 | 閲覧不可 | 閲覧のみ | 編集可 | 編集可 | 閲覧不可 |
| 配車担当者 | 閲覧不可 | 閲覧のみ | 閲覧不可 | 編集可 | 閲覧のみ |
| 計上担当者 | 閲覧不可 | 閲覧のみ | 閲覧不可 | 閲覧不可 | 編集可 |

### 2.5 ステータス値詳細

| ステータス値 | 表示名 | タグ色 | 説明 |
|--------------|--------|--------|------|
| 有効 | アカウント有効 | 緑色 | 账户正常使用中 |
| 無効 | アカウント無効 | 赤色 | 账户已禁用 |
| 編集可 | 〇 編集可 | 緑色 | 可编辑状态 |
| 閲覧のみ | △ 閲覧のみ | 橙色 | 仅查看状态 |
| 閲覧不可 | × 閲覧不可 | 赤色 | 不可访问状态 |

---

## 3. 交互流程

### 3.1 ページロードフロー

| ステップ | アクション | システム応答 |
|----------|-----------|--------------|
| 1 | ユーザーがb15ページへ遷移 | ページ初期化 |
| 2 | URLパラメータからuserId取得 | userId抽出 |
| 3 | 既存データ取得 | GET /api/v1/base-accounts/{userId} |
| 4 | ベース一覧取得 | GET /api/v1/base-list |
| 5 | APIレスポンス受信 | 各セクションに表示設定 |
| 6 | 権限マトリクス生成 | 権限区分に基づいて計算 |
| 7 | ページ表示完了 | 閲覧モードで表示 |

### 3.2 編集モード切替フロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | 編集ボタンクリック | 編集モードに切り替え |
| 2 | 入力フィールド有効化 | 各セクション入力可 |
| 3 | 保存ボタンクリック | バリデーション実行 |
| 4 | バリデーション成功 | 更新API実行 |
| 5 | 更新成功 | 保存完了メッセージ表示 |
| 6 | 閲覧モードに切り替え | 更新後のデータを表示 |

### 3.3 キャンセルフロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | キャンセルボタンクリック | 確認ダイアログ表示 |
| 2 | 確認 | 閲覧モードに切り替え |
| 3 | 原始データに復元 | フォーム値をリセット |

### 3.4 権限区分変更フロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | ラジオボタン選択変更 | 権限マトリクス再計算 |
| 2 | 権限マトリクス表示更新 | 該当権限区分に基づく行列表示 |

### 3.5 状态切换フロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | Switchクリック | 確認ダイアログ表示 |
| 2 | 確認 | 更新API実行 |
| 3 | 更新成功 | 状態表示更新、完了メッセージ |

### 3.6 所属変更フロー

| ステップ | ユーザー操作 | システム応答 |
|----------|-------------|--------------|
| 1 | ドロップダウン選択 | 所属ベース選択 |
| 2 | 保存ボタンクリック | バリデーション実行 |
| 3 | バリデーション成功 | 更新API実行 |
| 4 | 更新成功 | 完了メッセージ表示 |

---

## 4. 界面规范

### 4.1 ページレイアウト

| 属性 | PC端値 | SP端値 | 説明 |
|------|--------|--------|------|
| **ページタイプ** | PCのみ | - | PC专用页面 |
| **セクション配置** | 垂直配置 | 垂直配置 | ユーザー情報→所属→権限→権限マトリクス→状態 |
| **カード間隔** | 24px | 16px | カード間余白 |

### 4.2 ユーザー情報セクション仕様

| 属性 | 値 | 説明 |
|------|----|------|
| **セクションタイトル** | ユーザー情報 | 固定标题 |
| **フィールド配置** | グリッド配置 | 2列グリッド |
| **フィールド間隔** | 16px | フィールド間余白 |

### 4.3 所属セクション仕様

| 属性 | 値 | 説明 |
|------|----|------|
| **セクションタイトル** | 所属 | 固定标题 |
| **セレクター幅** | 300px | PC端固定幅 |

### 4.4 権限セクション仕様

| 属性 | 値 | 説明 |
|------|----|------|
| **セクションタイトル** | 権限 | 固定标题 |
| **ラジオボタン配置** | 垂直配置 | 各オプション垂直排列 |

### 4.5 権限マトリクス仕様

| 属性 | 値 | 説明 |
|------|----|------|
| **セクションタイトル** | 権限マトリクス | 固定标题 |
| **テーブル幅** | 100% | 幅一杯 |
| **列数** | 6列 | 機能＋5権限 |

### 4.6 アカウント状態セクション仕様

| 属性 | 値 | 説明 |
|------|----|------|
| **セクションタイトル** | アカウント有効・無効 | 固定标题 |
| **Switch配置** | 左配置 | 开关左对齐 |
| **説明文** | 右配置 | 説明文字右配置 |

### 4.7 ステータスタグ仕様

| ステータス | 背景色 | 文字色 | 説明 |
|-----------|--------|--------|------|
| 有効 | #52c41a | #ffffff | 绿色标签 |
| 無効 | #f5222d | #ffffff | 红色标签 |
| 編集可 | #52c41a | #ffffff | 绿色标签 |
| 閲覧のみ | #fa8c16 | #ffffff | 橙色标签 |
| 閲覧不可 | #f5222d | #ffffff | 红色标签 |

---

## 5. API接口设计

### 5.1 アカウント詳細取得API

```typescript
// GET /api/v1/base-accounts/{userId}
interface GetBaseAccountDetailRequest {
  userId: string;
}

interface GetBaseAccountDetailResponse {
  success: boolean;
  data: {
    userId: string;
    email: string;
    userName: string;
    affiliation: string;
    permission: PermissionType;
    permissionMatrix: PermissionMatrix;
    accountStatus: '有効' | '無効';
    azureAdId: string;
    createdAt: string;
    updatedAt: string;
    lastLoginAt?: string;
  };
  error?: {
    code: string;
    message: string;
  };
}

type PermissionType = '管理者' | '役割者' | '配車担当者' | '計上担当者';

interface PermissionMatrix {
  権限付与: '編集可' | '閲覧のみ' | '閲覧不可';
  承認: '編集可' | '閲覧のみ' | '閲覧不可';
  マスタ管理: '編集可' | '閲覧のみ' | '閲覧不可';
  発注: '編集可' | '閲覧のみ' | '閲覧不可';
  計上: '編集可' | '閲覧のみ' | '閲覧不可';
}
```

### 5.2 アカウント情報更新API

```typescript
// PUT /api/v1/base-accounts/{userId}
interface UpdateBaseAccountRequest {
  userId: string;
  updateData: {
    affiliation?: string;
    permission?: PermissionType;
    accountStatus?: '有効' | '無効';
  };
}

interface UpdateBaseAccountResponse {
  success: boolean;
  message: string;
}
```

### 5.3 ベース一覧取得API

```typescript
// GET /api/v1/base-list
interface GetBaseListResponse {
  success: boolean;
  data: {
    bases: BaseInfo[];
  };
}

interface BaseInfo {
  baseId: string;
  baseName: string;
  baseType: 'ベース' | '本社';
}
```

### 5.4 権限履歴取得API

```typescript
// GET /api/v1/base-accounts/{userId}/history
interface GetPermissionHistoryResponse {
  success: boolean;
  data: {
    history: PermissionHistoryItem[];
  };
}

interface PermissionHistoryItem {
  changeId: string;
  changedField: string;
  beforeValue: string;
  afterValue: string;
  changedBy: string;
  changedAt: string;
  changeReason?: string;
}
```

### 5.5 APIエラーレスポンス

```typescript
interface ApiErrorResponse {
  success: false;
  error: {
    code: ErrorCode;
    message: string;
    details?: Record<string, unknown>;
  };
}

type ErrorCode =
  | 'INVALID_PARAMETER'
  | 'ACCOUNT_NOT_FOUND'
  | 'INVALID_AFFILIATION'
  | 'INVALID_PERMISSION'
  | 'PERMISSION_DENIED'
  | 'LAST_ADMIN_ERROR'
  | 'SYSTEM_ERROR';
```

---

## 6. 前端实现

### 6.1 メインコンポーネント

```typescript
// BaseAccountDetailPage.tsx
import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import {
  Card,
  Form,
  Input,
  Select,
  Button,
  Radio,
  Switch,
  Table,
  Tag,
  message,
  Space,
  Divider,
  Typography,
  Row,
  Col,
  Alert,
} from 'antd';
import {
  EditOutlined,
  SaveOutlined,
  CloseOutlined,
  ArrowLeftOutlined,
  HistoryOutlined,
} from '@ant-design/icons';
import { BaseAccount, PermissionMatrix, PermissionType } from '../../types/baseAccount';
import { getBaseAccount, updateBaseAccount, getBaseList } from '../../api/baseAccount';
import styles from './BaseAccountDetailPage.module.css';

const { Title, Text, Paragraph } = Typography;

type Mode = 'view' | 'edit';

const permissionOptions: { value: PermissionType; label: string; color: string }[] = [
  { value: '管理者', label: '管理者', color: 'red' },
  { value: '役割者', label: '役割者', color: 'blue' },
  { value: '配車担当者', label: '配車担当者', color: 'green' },
  { value: '計上担当者', label: '計上担当者', color: 'orange' },
];

const permissionDisplay = {
  '編集可': { text: '〇 編集可', color: 'success' as const },
  '閲覧のみ': { text: '△ 閲覧のみ', color: 'warning' as const },
  '閲覧不可': { text: '× 閲覧不可', color: 'error' as const },
};

const buildPermissionMatrix = (permission: PermissionType): PermissionMatrix => {
  const matrices: Record<PermissionType, PermissionMatrix> = {
    管理者: {
      権限付与: '編集可',
      承認: '編集可',
      マスタ管理: '編集可',
      発注: '閲覧のみ',
      計上: '閲覧のみ',
    },
    役割者: {
      権限付与: '閲覧不可',
      承認: '閲覧のみ',
      マスタ管理: '編集可',
      発注: '編集可',
      計上: '閲覧不可',
    },
    配車担当者: {
      権限付与: '閲覧不可',
      承認: '閲覧のみ',
      マスタ管理: '閲覧不可',
      発注: '編集可',
      計上: '閲覧のみ',
    },
    計上担当者: {
      権限付与: '閲覧不可',
      承認: '閲覧のみ',
      マスタ管理: '閲覧不可',
      発注: '閲覧不可',
      計上: '編集可',
    },
  };
  return matrices[permission];
};

export const BaseAccountDetailPage: React.FC = () => {
  const { userId } = useParams<{ userId: string }>();
  const navigate = useNavigate();
  const [form] = Form.useForm();
  
  const [mode, setMode] = useState<Mode>('view');
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [data, setData] = useState<BaseAccount | null>(null);
  const [baseList, setBaseList] = useState<{ value: string; label: string }[]>([]);
  const [originalData, setOriginalData] = useState<BaseAccount | null>(null);

  const permission = Form.useWatch('permission', form) as PermissionType;

  useEffect(() => {
    if (userId) {
      fetchData();
    }
  }, [userId]);

  const fetchData = async () => {
    if (!userId) return;
    setLoading(true);
    try {
      const [accountResponse, baseListResponse] = await Promise.all([
        getBaseAccount(userId),
        getBaseList(),
      ]);

      if (accountResponse.success && accountResponse.data) {
        setData(accountResponse.data);
        setOriginalData(accountResponse.data);
        form.setFieldsValue(accountResponse.data);
      } else {
        message.error(accountResponse.error?.message || 'アカウント情報の取得に失敗しました');
      }

      if (baseListResponse.success && baseListResponse.data) {
        setBaseList(baseListResponse.data.bases.map(base => ({
          value: base.baseName,
          label: base.baseName,
        })));
      }
    } catch (error) {
      message.error('エラーが発生しました');
      console.error('Fetch error:', error);
    } finally {
      setLoading(false);
    }
  };

  const handleEdit = () => {
    setMode('edit');
    if (data) {
      form.setFieldsValue(data);
    }
  };

  const handleCancel = () => {
    setMode('view');
    if (originalData) {
      form.setFieldsValue(originalData);
    }
  };

  const handleSave = async () => {
    if (!userId) return;
    try {
      const values = await form.validateFields();
      setSaving(true);

      const response = await updateBaseAccount(userId, values);
      
      if (response.success) {
        message.success('保存しました');
        setMode('view');
        fetchData();
      } else {
        message.error(response.error?.message || '保存に失敗しました');
      }
    } catch (error) {
      message.error('保存中にエラーが発生しました');
      console.error('Save error:', error);
    } finally {
      setSaving(false);
    }
  };

  const matrix = permission ? buildPermissionMatrix(permission) : null;

  const matrixColumns = [
    {
      title: '',
      dataIndex: 'feature',
      key: 'feature',
      width: 150,
    },
    {
      title: '権限付与',
      dataIndex: 'permissionGrant',
      key: 'permissionGrant',
      width: 120,
      render: (value: string) => {
        const config = permissionDisplay[value as keyof typeof permissionDisplay];
        return <Tag color={config?.color}>{config?.text || value}</Tag>;
      },
    },
    {
      title: '承認',
      dataIndex: 'approval',
      key: 'approval',
      width: 120,
      render: (value: string) => {
        const config = permissionDisplay[value as keyof typeof permissionDisplay];
        return <Tag color={config?.color}>{config?.text || value}</Tag>;
      },
    },
    {
      title: 'マスタ管理',
      dataIndex: 'masterManagement',
      key: 'masterManagement',
      width: 120,
      render: (value: string) => {
        const config = permissionDisplay[value as keyof typeof permissionDisplay];
        return <Tag color={config?.color}>{config?.text || value}</Tag>;
      },
    },
    {
      title: '発注',
      dataIndex: 'ordering',
      key: 'ordering',
      width: 120,
      render: (value: string) => {
        const config = permissionDisplay[value as keyof typeof permissionDisplay];
        return <Tag color={config?.color}>{config?.text || value}</Tag>;
      },
    },
    {
      title: '計上',
      dataIndex: 'booking',
      key: 'booking',
      width: 120,
      render: (value: string) => {
        const config = permissionDisplay[value as keyof typeof permissionDisplay];
        return <Tag color={config?.color}>{config?.text || value}</Tag>;
      },
    },
  ];

  const matrixData = matrix ? [
    {
      key: 'current',
      feature: `【${permission || data?.permission}】の権限`,
      permissionGrant: matrix.権限付与,
      approval: matrix.承認,
      masterManagement: matrix.マスタ管理,
      ordering: matrix.発注,
      booking: matrix.計上,
    },
  ] : [];

  if (loading) {
    return <div className={styles.loading}>Loading...</div>;
  }

  if (!data) {
    return <div className={styles.error}>アカウントが見つかりません</div>;
  }

  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <Space>
          <Button
            icon={<ArrowLeftOutlined />}
            onClick={() => navigate(-1)}
          >
            戻る
          </Button>
          <Title level={2}>ベースアカウントマスタ詳細</Title>
        </Space>
        <Space>
          {mode === 'view' ? (
            <Button
              type="primary"
              icon={<EditOutlined />}
              onClick={handleEdit}
            >
              編集
            </Button>
          ) : (
            <>
              <Button
                type="primary"
                icon={<SaveOutlined />}
                onClick={handleSave}
                loading={saving}
              >
                保存
              </Button>
              <Button
                icon={<CloseOutlined />}
                onClick={handleCancel}
              >
                キャンセル
              </Button>
            </>
          )}
        </Space>
      </div>

      <Form
        form={form}
        layout="vertical"
        disabled={mode === 'view'}
      >
        <Card title="ユーザー情報" className={styles.section}>
          <Row gutter={24}>
            <Col xs={24} sm={12} md={8}>
              <div className={styles.infoItem}>
                <Text type="secondary">ユーザーID</Text>
                <Text strong>{data.userId}</Text>
              </div>
            </Col>
            <Col xs={24} sm={12} md={8}>
              <div className={styles.infoItem}>
                <Text type="secondary">メール</Text>
                <Text strong>{data.email}</Text>
              </div>
            </Col>
            <Col xs={24} sm={12} md={8}>
              <div className={styles.infoItem}>
                <Text type="secondary">氏名</Text>
                <Text strong>{data.userName}</Text>
              </div>
            </Col>
          </Row>
        </Card>

        <Card title="所属" className={styles.section}>
          <Form.Item
            name="affiliation"
            label="所属ベース"
            rules={[{ required: true, message: '所属を選択してください' }]}
          >
            <Select
              placeholder="ベースまたは，本社を選択"
              options={baseList}
              className={styles.select}
            />
          </Form.Item>
        </Card>

        <Card title="権限" className={styles.section}>
          <Form.Item
            name="permission"
            label="権限区分"
            rules={[{ required: true, message: '権限を選択してください' }]}
          >
            <Radio.Group className={styles.permissionRadioGroup}>
              {permissionOptions.map(option => (
                <Radio.Button key={option.value} value={option.value}>
                  <Tag color={option.color}>{option.label}</Tag>
                </Radio.Button>
              ))}
            </Radio.Group>
          </Form.Item>
        </Card>

        <Card title="権限マトリクス" className={styles.section}>
          <Table
            columns={matrixColumns}
            dataSource={matrixData}
            rowKey="key"
            pagination={false}
            size="small"
            className={styles.permissionMatrixTable}
          />
        </Card>

        <Card title="アカウント有効・無効" className={styles.section}>
          <Form.Item
            name="accountStatus"
            valuePropName="checked"
          >
            <Switch
              checkedChildren="有効"
              unCheckedChildren="無効"
              className={styles.statusSwitch}
            />
          </Form.Item>
          <Paragraph type="secondary">
            {form.getFieldValue('accountStatus') !== false
              ? '本システムを利用できる状態です'
              : '本システムを利用できない状態です'}
          </Paragraph>
        </Card>
      </Form>

      <Card className={styles.historySection} extra={
        <Button icon={<HistoryOutlined />}>履歴詳細</Button>
      }>
        <div className={styles.timestamp}>
          <Text type="secondary">
            最終更新: {data.updatedAt} | 作成日: {data.createdAt}
          </Text>
        </div>
      </Card>
    </div>
  );
};
```

### 6.2 APIクライアント

```typescript
// baseAccountApi.ts
import axios from 'axios';
import {
  GetBaseAccountDetailRequest,
  GetBaseAccountDetailResponse,
  UpdateBaseAccountRequest,
  UpdateBaseAccountResponse,
  GetBaseListResponse,
} from '../types/baseAccount';

const API_BASE_URL = '/api/v1';

const apiClient = axios.create({
  baseURL: API_BASE_URL,
  timeout: 30000,
  headers: {
    'Content-Type': 'application/json',
  },
});

apiClient.interceptors.request.use((config) => {
  const token = localStorage.getItem('auth_token');
  if (token) {
    config.headers.Authorization = `Bearer ${token}`;
  }
  return config;
});

apiClient.interceptors.response.use(
  (response) => response.data,
  (error) => {
    if (error.response?.status === 401) {
      window.location.href = '/login';
    }
    return Promise.reject(error);
  }
);

export const getBaseAccount = async (
  userId: string
): Promise<GetBaseAccountDetailResponse> => {
  return apiClient.get(`/base-accounts/${userId}`);
};

export const updateBaseAccount = async (
  userId: string,
  data: UpdateBaseAccountRequest['updateData']
): Promise<UpdateBaseAccountResponse> => {
  return apiClient.put(`/base-accounts/${userId}`, { updateData: data });
};

export const getBaseList = async (): Promise<GetBaseListResponse> => {
  return apiClient.get('/base-list');
};
```

---

## 7. 后端服务设计

### 7.1 Controller

```java
@RestController
@RequestMapping("/api/v1/base-accounts")
@RequiredArgsConstructor
@Validated
public class BaseAccountController {

    private final BaseAccountService baseAccountService;

    @GetMapping("/{userId}")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<GetBaseAccountDetailResponse> getAccountDetail(
            @PathVariable @NotBlank String userId) {
        GetBaseAccountDetailResponse response = baseAccountService.getAccountDetail(userId);
        return ResponseEntity.ok(response);
    }

    @PutMapping("/{userId}")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<UpdateBaseAccountResponse> updateAccount(
            @PathVariable @NotBlank String userId,
            @Valid @RequestBody UpdateBaseAccountRequest request) {
        UpdateBaseAccountResponse response = baseAccountService.updateAccount(
            userId, request.getUpdateData());
        return ResponseEntity.ok(response);
    }

    @GetMapping("/base-list")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<GetBaseListResponse> getBaseList() {
        GetBaseListResponse response = baseAccountService.getBaseList();
        return ResponseEntity.ok(response);
    }
}
```

### 7.2 Service

```java
@Service
@RequiredArgsConstructor
@Slf4j
public class BaseAccountService {

    private final BaseAccountRepository baseAccountRepository;
    private final BaseListRepository baseListRepository;

    @Transactional(readOnly = true)
    public GetBaseAccountDetailResponse getAccountDetail(String userId) {
        BaseAccount account = baseAccountRepository.findById(userId)
                .orElseThrow(() -> new AccountNotFoundException(userId));

        PermissionMatrix matrix = buildPermissionMatrix(account.getPermission());

        return GetBaseAccountDetailResponse.builder()
                .success(true)
                .data(BaseAccountData.builder()
                        .userId(account.getUserId())
                        .email(account.getEmail())
                        .userName(account.getUserName())
                        .affiliation(account.getAffiliation())
                        .permission(account.getPermission().getValue())
                        .permissionMatrix(matrix)
                        .accountStatus(account.getAccountStatus().getValue())
                        .azureAdId(account.getAzureAdId())
                        .createdAt(account.getCreatedAt().toString())
                        .updatedAt(account.getUpdatedAt().toString())
                        .lastLoginAt(account.getLastLoginAt()?.toString())
                        .build())
                .build();
    }

    @Transactional
    public UpdateBaseAccountResponse updateAccount(
            String userId, UpdateBaseAccountRequest.UpdateData updateData) {
        BaseAccount account = baseAccountRepository.findById(userId)
                .orElseThrow(() -> new AccountNotFoundException(userId));

        if (updateData.getAffiliation() != null) {
            account.setAffiliation(updateData.getAffiliation());
        }

        if (updateData.getPermission() != null) {
            PermissionType newPermission = PermissionType.fromValue(updateData.getPermission());
            
            if (newPermission != account.getPermission()) {
                long adminCount = baseAccountRepository.countByPermission(PermissionType.管理者);
                if (account.getPermission() == PermissionType.管理者 && adminCount <= 1) {
                    throw new LastAdminException("最後の管理者の権限を変更することはできません");
                }
            }
            account.setPermission(newPermission);
        }

        if (updateData.getAccountStatus() != null) {
            AccountStatus newStatus = AccountStatus.fromValue(updateData.getAccountStatus());
            
            if (newStatus != account.getAccountStatus()) {
                long adminCount = baseAccountRepository.countByPermissionAndStatus(
                    PermissionType.管理者, AccountStatus.有効);
                if (account.getPermission() == PermissionType.管理者 && 
                    newStatus == AccountStatus.無効 && adminCount <= 1) {
                    throw new LastAdminException("最後の管理者を無効にすることはできません");
                }
            }
            account.setAccountStatus(newStatus);
        }

        baseAccountRepository.save(account);

        log.info("ベースアカウント {} の情報が更新されました", userId);

        return UpdateBaseAccountResponse.builder()
                .success(true)
                .message("保存しました")
                .build();
    }

    private PermissionMatrix buildPermissionMatrix(PermissionType permission) {
        Map<String, String> matrix = new HashMap<>();

        switch (permission) {
            case 管理者:
                matrix.put("権限付与", "編集可");
                matrix.put("承認", "編集可");
                matrix.put("マスタ管理", "編集可");
                matrix.put("発注", "閲覧のみ");
                matrix.put("計上", "閲覧のみ");
                break;
            case 役割者:
                matrix.put("権限付与", "閲覧不可");
                matrix.put("承認", "閲覧のみ");
                matrix.put("マスタ管理", "編集可");
                matrix.put("発注", "編集可");
                matrix.put("計上", "閲覧不可");
                break;
            case 配車担当者:
                matrix.put("権限付与", "閲覧不可");
                matrix.put("承認", "閲覧のみ");
                matrix.put("マスタ管理", "閲覧不可");
                matrix.put("発注", "編集可");
                matrix.put("計上", "閲覧のみ");
                break;
            case 計上担当者:
                matrix.put("権限付与", "閲覧不可");
                matrix.put("承認", "閲覧のみ");
                matrix.put("マスタ管理", "閲覧不可");
                matrix.put("発注", "閲覧不可");
                matrix.put("計上", "編集可");
                break;
        }

        return PermissionMatrix.builder()
                .権限付与(matrix.get("権限付与"))
                .承認(matrix.get("承認"))
                .マスタ管理(matrix.get("マスタ管理"))
                .発注(matrix.get("発注"))
                .計上(matrix.get("計上"))
                .build();
    }
}
```

---

## 8. 数据库表设计

### 8.1 base_accountsテーブル

```sql
CREATE TABLE base_accounts (
    id BIGSERIAL PRIMARY KEY,
    user_id VARCHAR(50) NOT NULL UNIQUE COMMENT 'ユーザーID',
    email VARCHAR(200) NOT NULL UNIQUE COMMENT 'メールアドレス',
    user_name VARCHAR(200) NOT NULL COMMENT '氏名',
    affiliation VARCHAR(200) NOT NULL COMMENT '所属ベース',
    permission ENUM('管理者', '役割者', '配車担当者', '計上担当者') NOT NULL DEFAULT '役割者' COMMENT '権限区分',
    account_status ENUM('有効', '無効') NOT NULL DEFAULT '有効' COMMENT 'アカウント状態',
    azure_ad_id VARCHAR(100) COMMENT 'AzureADオブジェクトID',
    roles TEXT[] COMMENT '役割リスト',
    permissions TEXT[] COMMENT '権限リスト',
    last_login_at TIMESTAMP COMMENT '最終ログイン日時',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    
    INDEX idx_base_account_user_id (user_id),
    INDEX idx_base_account_email (email),
    INDEX idx_base_account_affiliation (affiliation),
    INDEX idx_base_account_permission (permission),
    INDEX idx_base_account_status (account_status),
    INDEX idx_base_account_permission_status (permission, account_status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='ベースアカウントマスタテーブル';
```

### 8.2 base_listテーブル

```sql
CREATE TABLE base_list (
    id BIGSERIAL PRIMARY KEY,
    base_id VARCHAR(50) NOT NULL UNIQUE COMMENT 'ベースID',
    base_name VARCHAR(200) NOT NULL COMMENT 'ベース名',
    base_type ENUM('ベース', '本社') NOT NULL COMMENT 'ベースタイプ',
    address TEXT COMMENT '住所',
    phone VARCHAR(20) COMMENT '電話番号',
    is_active BOOLEAN NOT NULL DEFAULT TRUE COMMENT '有効状態',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    
    INDEX idx_base_list_type (base_type),
    INDEX idx_base_list_active (is_active)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='ベースリストテーブル';
```

### 8.3 base_account_audit_logテーブル

```sql
CREATE TABLE base_account_audit_log (
    id BIGSERIAL PRIMARY KEY,
    user_id VARCHAR(50) NOT NULL COMMENT '対象ユーザーID',
    changed_field VARCHAR(50) NOT NULL COMMENT '変更フィールド',
    before_value TEXT COMMENT '変更前値',
    after_value TEXT COMMENT '変更後値',
    change_reason TEXT COMMENT '変更理由',
    performed_by VARCHAR(100) NOT NULL COMMENT '実行者',
    performed_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    
    INDEX idx_audit_user_id (user_id),
    INDEX idx_audit_performed_at (performed_at),
    
    FOREIGN KEY (user_id) REFERENCES base_accounts(user_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='ベースアカウント監査ログテーブル';
```

---

## 9. 验收标准

### 9.1 功能验收

| 测试场景 | 预期结果 | 优先级 |
|----------|----------|--------|
| 页面初始加载（既存用户） | 显示用户信息、所属、权限、矩阵 | P0 |
| 页面URL访问 | 正确获取userId并加载数据 | P0 |
| 閲覧モード表示 | 只读字段不可编辑 | P0 |
| 編集モード切替 | 点击编辑按钮后变为可编辑 | P0 |
| 所属変更 | 可以选择并保存所属ベース | P0 |
| 権限区分選択 | 可以选择并保存権限区分 | P0 |
| 権限マトリクス表示 | 根据権限区分正确显示矩阵 | P0 |
| 保存按钮点击 | 验证并提交更新 | P0 |
| キャンセル按钮点击 | 恢复原始值，退出编辑模式 | P0 |
| 有効/無効切换 | 状态正确切换 | P0 |
| バリデーション | 必填项验证 | P0 |
| 最終管理者チェック | 最后一个管理者不能被禁用 | P0 |

### 9.2 视觉验收

| 项目 | 要求 |
|------|------|
| 布局 | 与设计稿一致 |
| 颜色 | 权限标签颜色正确 |
| 间距 | 符合UI规范 |
| 字体 | 大小、颜色正确 |
| 矩阵显示 | 权限矩阵显示正确 |

### 9.3 性能验收

| 指标 | 要求 | 测试方法 |
|------|------|----------|
| 页面加载 | <2秒 | Lighthouse |
| API响应 | <500ms | API监控 |

### 9.4 安全验收

| 检查项 | 要求 |
|--------|------|
| 权限控制 | 只有管理者能访问 |
| CSRF防护 | 所有PUT请求带Token |
| 只读字段保护 | 后端验证只读字段不可修改 |
| SQL注入 | 使用参数化查询 |

---

## 10. 变更日志

| 日期 | 版本 | 变更内容 | 作者 |
|------|------|---------|------|
| 2026-02-XX| v1.0 | 初始版本 | - |

---

## 附录

### A.1 相关文档

| 文档 | 路径 | 说明 |
|------|------|------|
| 公共技术架构规范 | [\_公共技术架构规范.md](../_公共技术架构规范.md) | 前后端技术架构详细规范 |
| b14-ベースアカウントマスタ検索 | [../b14-ベースアカウントマスタ検索/05_设计规范.md](../b14-ベースアカウントマスタ検索/05_设计规范.md) | 账户搜索页面 |
| b11-輸送パートナーマスタ一覧 | [../b11-輸送パートナーマスタ一覧/05_设计规范.md](../b11-輸送パートナーマスタ一覧/05_设计规范.md) | 合作伙伴公司一览 |
| AzureAD連携仕様 | ../docs/azuread_integration.md | AzureAD連携详细说明 |

### A.2 関連画面遷移

| 画面 | 遷移元 | 遷移先 | トリガー | 説明 |
|------|--------|--------|----------|------|
| b15 | b14一覧 | - | - | 一覧から選択 |
| b15 | - | b14 | 戻るボタン | 検索画面へ戻る |

### A.3 権限マトリクス定義

| 権限区分 | 権限付与 | 承認 | マスタ管理 | 発注 | 計上 |
|----------|----------|------|------------|------|------|
| 管理者 | 編集可 | 編集可 | 編集可 | 閲覧のみ | 閲覧のみ |
| 役割者 | 閲覧不可 | 閲覧のみ | 編集可 | 編集可 | 閲覧不可 |
| 配車担当者 | 閲覧不可 | 閲覧のみ | 閲覧不可 | 編集可 | 閲覧のみ |
| 計上担当者 | 閲覧不可 | 閲覧のみ | 閲覧不可 | 閲覧不可 | 編集可 |

### A.4 API调用示例

```bash
# アカウント詳細取得
curl -X GET "https://api.example.com/api/v1/base-accounts/U001" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
```

```bash
# アカウント情報更新
curl -X PUT "https://api.example.com/api/v1/base-accounts/U001" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..." \
  -H "Content-Type: application/json" \
  -d '{
    "updateData": {
      "affiliation": "東京ベース",
      "permission": "配車担当者",
      "accountStatus": "有効"
    }
  }'
```

```bash
# ベース一覧取得
curl -X GET "https://api.example.com/api/v1/base-list" \
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
```

---

**文档编制**: -  
**编制时间**: 2026-02-XX 
**适用范围**: b15-ベースアカウントマスタ詳細 基础账户详情页面  
**交叉引用**: [\_公共技术架构规范.md](../_公共技术架构规范.md)
